...
class Thesaurus:
    def __init__(self, lang, user_config=None):
        ...
        self.download('som_file', 'som_url', 'som_path')
        # download background embeddings
        self.download('back_embeds', 'embeds_url', 'back_embeds_path')
    ...
    def download(self, file_key, url_key, path_key):
        ...
        if url_key in self.user_config:
            ...
            output_directory = self.paths[path_key]
    ...
    def set_som(self):
        if 'som_file' not in self.user_config:
            som = pkg_resources.read_binary(self.paths['som_path'], self.config['som_file'])
            som = pickle.loads(som)
        else:
            model = open(self.user_config['som_file'], 'rb')
    ...
    def import_background(self, b_tokens=None, b_embeds=None):
        ...
        if b_tokens is None and b_embeds is None:
            ...
            background_embeds = pkg_resources.read_binary(self.paths['back_embeds_path'], self.config['back_embeds'])
