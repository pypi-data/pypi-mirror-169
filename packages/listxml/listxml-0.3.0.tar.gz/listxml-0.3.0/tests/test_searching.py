...
class TestSearching(unittest.TestCase):
    ...
    def test_attributes_list(self):
        content = ['div',
                   ...
                   ['p', [], "p2"],
                   ['p', [['a1', 'v1']], "p3"],
                   ['p', [['x', 'nothing']], ['p', [['a1', 'v2']], 'x'], "p4"],
                   ['other', [['a1', 'v3']]]]
        self.assertEqual(search_for_path(['p', ['a1']], content),
                         ['v1', 'v2'])

    # here, also confirm that with_element has no effect when searching for attributes
    ...
    def test_edges(self):
        ...
        with self.assertRaises(ValueError):
            self.assertEqual(search_for_path(['p', []], ['p']), [])
        with self.assertRaises(ValueError):
            self.assertEqual(search_for_path(['p', ['a', 'b']], ['p']), [])
        # should we regard the following as malformed, instead of 'match nothing'?
        self.assertEqual(search_for_path(['p', []], ['div']), [])
