...
def _delvewheel_init_patch_1_0_0():
    import os
    import sys
    ...
    if sys.version_info[:2] >= (3, 8) and not os.path.exists(os.path.join(sys.base_prefix, 'conda-meta')) or sys.version_info[:2] >= (3, 10):
        ...
        with open(os.path.join(libs_dir, '.load-order-tesseract_robotics-0.1.7.post2')) as file:
            load_order = file.read().split()
        for lib in load_order:
            WinDLL(os.path.join(libs_dir, lib))


_delvewheel_init_patch_1_0_0()
del _delvewheel_init_patch_1_0_0
