...
class Quip:
    ...
    def dump_fields(self, write=False):
        ...
        if os.path.exists(template):
            with open(template, "r") as f:
                ...
                template_dict["fields"] = fields_dict
                events_dict = fb.dump_events(_json.get("events"))
                template_dict["events"] = events_dict
    ...
    def create_external_systems(self):
        sq_config = self._global_conf_external.get("sonarqube", {})
        ...
        prefix = sq_config.get("prefix", "cs-uac")
        sq_project_name = prefix + "-" + self.extension_name
