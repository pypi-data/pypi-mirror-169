...
class PIH:
    ...
    class SERVICE:
        ...
        def ping(role: ServiceRoles) -> ServiceRoleBaseValue:
            info = RPC.ping(role)
            if info:
                service_role_info: ServiceInformation = info
                DataTools.fill_data_from_source(role.value, DataTools.to_data(service_role_info))
            return info
    ...
    class SESSION:
        ...
        def app_restart() -> None:
            #,  stdout=DEVNULL, stderr=STDOUT, text=True
            #file_path: str =  PIH.SESSION.get_file_path()
            os.execl(sys.executable, sys.executable, *sys.argv)
            #os.execv(PIH.SESSION.get_file_path(), sys.argv)
            #subprocess.run([CONST.PYTHON.EXECUTOR, file_path])
            #exit()
    ...
    class MESSAGE:
        ...
        class COMMAND:
            ...
            def service_started(role: ServiceRoles) -> bool:
                service_role_value: ServiceRoleValue = role.value
                return PIH.log_command(LogCommands.SERVICE_STARTED, (role.name, service_role_value.description, service_role_value.host, service_role_value.port, PIH.OS.get_pid()))
    ...
    class VISUAL:
        ...
        def service_header(role: ServiceRoles) -> None:
            ...
            PR.blue(f"PIH локальная версия: {PIH.VERSION.local()}")
            ...
            PR.green(f"Service pid: {PIH.OS.get_pid()}")
