    ...
    class UPDATER:
        ...
        def update_for_role(value: ServiceRoles, show_output: bool = False, by_version: bool = True) -> bool:
            ...
            command_list: List[str] = remote_executor_command_list + \
                PIH.UPDATER.get_updater_command_list(by_version)
        ...
        @staticmethod
        def get_updater_command_list(by_version: bool= True) -> List[str]:
            return ["-m", CONST.PYTHON.PYPI, "install"] + ([f"{PIH.NAME}=={PIH.VERSION.remote()}"] if by_version else [PIH.NAME, "-U"])

        @staticmethod
        def update_localy(show_output: bool = False) -> bool:
    ...
    class INPUT:
        ...
        def index(caption: str, data: dict, item_label: Callable = None) -> int:
            ...
            while True:
                ...
                if length == 1:
                    return 0
                selected_index = PIH.INPUT.input(PR.input_str(
    ...
    class CHECK:
        ...
        class SERVICE:
            ...
            class ADMIN:
                ...
                @staticmethod
                def availiable() -> bool:
                    return PIH.CHECK.RIGTH.by_group(CONST.AD.Groups.ServiceAdmin)
            ...
            def it_notify_about_new_user(login: str, password: str) -> bool:
                ...
                return PIH.log_command(LogCommands.IT_NOTIFY_ABOUT_NEW_USER, (user.name, user.description, user.samAccountName, password, user.telephoneNumber, user.mail, mark.TabNumber)) and \
                    PIH.log_command(LogCommands.IT_TASK_AFTER_CREATE_NEW_USER, (FullNameTool.to_given_name(
                        FullNameTool.from_string(it_user.name)), user.name, user.mail, password))
