...
class RPC:
    ...
    class Service:
        ...
        def serve_role(role: ServiceRoles, handler: Callable, start_handler: Callable = None, test: bool = False) -> None:
            ...
            installed_modules: List[str] = {pkg.key.lower() for pkg in pkg_resources.working_set}
            for module_name in [item.lower() for item in service_role_value.modules]:
                if module_name not in installed_modules:
                    ...
                    PR.alert(f"Модуль '{module_name}' не найден!")
    ...
    def call(command: rpcCommand, parameters: Any = None) -> str:
        ...
        try:
            ...
            PIH.ERROR.rpc_error_handler(details, code, None, command)
