...
class PIH:
    ...
    class UPDATER:
        ...
        def update_for_service(service_role: ServiceRoles, pih_update: bool = True, modules_update: bool = True, show_output: bool = False) -> bool:
            ...
            if modules_update and result:
                installed_module_list: List[str] = {pkg.key.lower() for pkg in pkg_resources.working_set}
                for module_name in [item.lower() for item in service_role_value.modules]:
                    if module_name not in installed_module_list:
                        result = result and PIH.UPDATER.update_module_localy(module_name, show_output=show_output)
                        if result:
    ...
    class SERVICE:
        ...
        class ADMIN:
            ...
            def start(role: ServiceRoles, check_if_started: bool = True, test: bool = False, show_output: bool = False) -> bool:
                ...
                if show_output:
                    if not DataTools.is_empty(process_result.stderr):
                        PR.write_line(str(process_result.stderr).replace(
                            "\n\n\n", "\n").replace("\n\n", "\n"))
                returncode = process_result.returncode
                if returncode == 2:
                    return False
                ...
                if CONST.SESSION.USE_AUTHENTIFICATION:
                    ...
                    try:
                        if RPC_COMMANDS.USER.authenticate(login, password):
                            ...
                            PR.good(PR.text_black(f"Добро пожаловать, {PIH.SESSION.get_user().name}..."))
                            return True
                        else:
    ...
    class VISUAL:
            ...
            def result(result: Result, root_location: str = CONST.AD.ACTIVE_USERS_CONTAINER_DN) -> None:
                ...
                def modify_data(field: FieldItem, user: User) -> str:
                    if field.name == USER_PROPERTY.DN:
                        ...
                        return None
                PIH.VISUAL.table_with_caption(result, "Пользватели:", False, None, modify_data)
        ...
        def service_header(role: ServiceRoles) -> None:
            ...
            PR.blue(f"PIH установленная версия: {service_role_value.pih_version}")
