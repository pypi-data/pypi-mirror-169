...
class PIH:
    ...
    class UPDATER:
        ...
        @staticmethod
        def update_for_role(value: ServiceRoles) -> bool:
            ...
            process_result = subprocess.run(remote_executor_command_list + ["-m", CONST.PYTHON.PYPI, "install", f"{PIH.NAME}=={PIH.VERSION.remote()}"],  stdout=DEVNULL,
                                            stderr=STDOUT, text=True)
            returncode = process_result.returncode
            return returncode == 0

        def get_updater_command_list() -> List[str]:
            return [CONST.PYTHON.EXECUTOR, "-m", CONST.PYTHON.PYPI,
                "install", f"{PIH.NAME}=={PIH.VERSION.remote()}"]

        @staticmethod
        def update_localy() -> bool:
            #,  stdout=DEVNULL,  stderr=STDOUT, text=True
            process_result = subprocess.run(PIH.UPDATER.get_updater_command_list())
            returncode = process_result.returncode
            return returncode == 0

        @staticmethod
        def update_remote(host: str) -> bool:
            #,  stdout=DEVNULL,  stderr=STDOUT, text=True
            remote_executor_command_list: List[str] = PIH.PSTOOLS.create_remote_executor(host, True)
            process_result = subprocess.run(
                remote_executor_command_list + PIH.UPDATER.get_updater_command_list())
            returncode = process_result.returncode
            return returncode == 0
    ...
    class SERVICE:
        ...
        def start(role: ServiceRoles, check_if_started: bool = True, test: bool = False) -> bool:
            ...
            process_result = subprocess.run(remote_executor_command_list.append(service_file_path), stdout=DEVNULL,
                stderr=STDOUT, text=True)
            returncode = process_result.returncode
            if returncode == 2:
                return False
