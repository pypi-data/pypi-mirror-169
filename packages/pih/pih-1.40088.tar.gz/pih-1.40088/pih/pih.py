...
class PIH:
    ...
    class UPDATER:
        ...
        @staticmethod
        def update_action(start_handler: Callable, update_complete_handler: Callable) -> None:
            need_update: bool = PIH.VERSION.need_update()
            def internal_update_action(need_update: bool, start_handler: Callable, update_complete_handler: Callable):
                start_handler(need_update)
                if need_update and PIH.UPDATER.update_localy(True):
                    import importlib
                    importlib.reload(sys.modules[PIH.NAME])
                    importlib.reload(sys.modules[f"{PIH.NAME}.{PIH.NAME}"])
                    update_complete_handler()    
            Thread(target=internal_update_action, args=(need_update, start_handler, update_complete_handler,)).start() 
