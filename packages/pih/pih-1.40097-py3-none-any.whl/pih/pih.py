...
class PIH:
    ...
    class ERROR:
        ...
        def rpc_error_handler(details: str, code: Tuple, role: ServiceRoles, command: ServiceCommands) -> None:
            if isinstance(command, ServiceCommands) and (role != ServiceRoles.LOG or code != StatusCode.UNAVAILABLE):
                PIH.MESSAGE.to_system_group_from_debug_bot(PIH.ERROR.create_error_header(details), LogLevels.ERROR)
    ...
    class UPDATER:
        ...
        @staticmethod
        def update_for_role(value: ServiceRoles, show_output: bool = False, by_version: bool = True) -> bool:
            service_role_value: ServiceRoleValue = value.value
            ...
            command_list: List[str] = remote_executor_command_list + \
                PIH.UPDATER.get_module_updater_command_list(PIH.NAME, PIH.VERSION.remote() if by_version else None)
        ...
        def update_localy(version: str = None, show_output: bool = False) -> bool:
            return PIH.UPDATER.uodate_module_localy(PIH.NAME, version, show_output)

        def uodate_module_localy(module_name: str, version: str = None, show_output: bool = False) -> bool:
        ...
        def update_action(start_handler: Callable, update_start_handler: Callable, update_complete_handler: Callable) -> None:
            ...
            def internal_update_action(need_update: bool, start_handler: Callable, update_start_handler: Callable, update_complete_handler: Callable):
                if need_update:
                    ...
                    if PIH.UPDATER.update_localy(True):
                        import importlib
    ...
    class PSTOOLS:
        ...
        def run_command(command_list: List[str], show_output: bool) -> CompletedProcess:
            if show_output:
                process_result = subprocess.run(command_list, text=True)
            else:
    ...
    class SESSION:
        ...
        def get_user() -> User:
            if PIH.SESSION.user is None:
                PIH.SESSION.user = PIH.RESULT.USER.by_login(PIH.SESSION.get_login(), True).data
        ...
        def authenticate(exit_on_fail: bool = False) -> bool:
            try:
                if CONST.SESSION.USE_AUTHENTIFICATION:
        ...
        class USER:
            ...
            def by_any(value: Any) -> Result[List[User]]:
                if isinstance(value, FullName):
                    ...
                    if value == "" or PIH.CHECK.login(value):
    ...
    class INPUT:
        ...
        def input(caption: str = None, new_line: bool = True) -> str:
            try:
                if new_line and caption is not None:
                    PR.input(caption)
                return input() if new_line else input(caption)
            except KeyboardInterrupt:
        ...
        def tab_number(check: bool = True) -> str:
            tab_number: str = None
            while True:
                tab_number = PIH.INPUT.input("Введите табельный номер:")
                if check:
                    if PIH.CHECK.tab_number(tab_number):
                        return tab_number
                    else:
                        PR.alert("Неправильный формат табельного номера")
                        return None
                else:
                    return tab_number
        ...
        def yes_no(text: str = " ", enter_for_yes: bool = False) -> bool:
            ...
            print(f"{text} \n{PR.green_str(PR.text_color(Fore.WHITE,'Да (1 или Ввод)'))} / {PR.red_str('Нет (Остальное)')}" if enter_for_yes else
                  f"{text} \n{PR.red_str('Да (1)')} / {PR.green_str(PR.text_color(Fore.WHITE, 'Нет (Остальное или Ввод)'))}")
                ...
                if user_list is not None:
                    for user in user_list:
                        if user.samAccountName == login:
                            ...
                            result = True
                            break
                PIH.MESSAGE.to_system_group_from_system_bot(f"Запрос на доступа к группе: {group.name} от пользователя {PIH.SESSION.get_user().name} ({login}). Доступ {'разрешен' if result else 'отклонен'}.", LogLevels.NORMAL if result else LogLevels.ERROR)
    ...
    def log(message: str, channel: LogChannels = LogChannels.DEFAULT, level: Any = LogLevels.DEFAULT) -> None:
        ...
        def internal_log(message: str, channel_name: str, level_value: int) -> None:
            try:
                RPC.call_by_role(ServiceCommands.log,
    ...
    class MESSAGE:
        ...
        class WHATS_APP:
            ...
            def send_message(telephoneNumber: str, message: str) -> bool:
                ...
                if not pywhatkit_is_exists:
                    PR.green("Установка библиотеки для отправки сообщения. Ожидайте...")
                    if not PIH.UPDATER.uodate_module_localy("pywhatkit"):
                        PR.alert(
        ...
        class COMMAND:
            ...
            def login() -> bool:
                ...
                return PIH.log_command(LogCommands.LOG_IN, (user.name, login, PIH.OS.get_host()))
            ...
            def start_session() -> bool:
                ...
                login: str = PIH.SESSION.get_login()
                user: User = PIH.RESULT.USER.by_login(login, True).data
                return PIH.log_command(LogCommands.START_SESSION, (user.name, login, f"{PIH.SESSION.get_file_name()} {argv_str}", f"{PIH.VERSION.local()}/{PIH.VERSION.remote()}", PIH.OS.get_host()))
            ...
            def hr_notify_about_new_employee(login: User) -> bool:
                ...
                return PIH.log_command(LogCommands.HR_NOTIFY_ABOUT_NEW_EMPLOYEE, (FullNameTool.to_given_name(hr_user.name),
                                                                                  user.name, user.mail))
            ...
            def it_notify_about_new_user(login: str, password: str) -> bool:
                ...
                return PIH.log_command(LogCommands.IT_NOTIFY_ABOUT_NEW_USER, (user.name, user.description, user.samAccountName, password, user.telephoneNumber, user.mail, mark.TabNumber)) and \
                    PIH.log_command(LogCommands.IT_TASK_AFTER_CREATE_NEW_USER, (FullNameTool.to_given_name(it_user.name), user.name, user.mail, password))
    ...
    class VISUAL:
        ...
        def pih_title() -> None:
            ...
            PR.cyan(
                 PR.text_color(Fore.WHITE, "█▄█ ┼█┼ █▄█"))
            PR.cyan(PR.text_color(Fore.WHITE, "█┼┼ ▄█▄ █┼█") + PR.text_color(Fore.BLACK, f" {PIH.VERSION.local()}"))
            PR.empty_line()
            ...
            def result(result: Result, root_location: str = CONST.AD.ACTIVE_USERS_CONTAINER_DN) -> None:
                ...
                def modify_data(field: FieldItem, user: User) -> str:
                    if field.name == USER_PROPERTY.DN:
                        ...
                        return DataTools.get_first(user.description, "")
                    elif field.name == USER_PROPERTY.NAME:
                        return "\n".join(user.name.split(" "))
                    else:
                        return None
                PIH.VISUAL.table_with_caption(result, None, False, None, modify_data)
        ...
        def service_header(role: ServiceRoles) -> None:
            service_role_value: ServiceRoleValue = role.value
            PR.blue("Запуск PIH сервиса")
            PR.blue(f"PIH локальная версия: {service_role_value.pih_version}")
            PR.blue(f"PIH версия: {PIH.VERSION.remote()}")
            PR.green(f"Хост: {service_role_value.host}")
            PR.green(f"Порт: {service_role_value.port}")
            PR.green(f"Имя хоста: {service_role_value.description}")
    ...
    class ACTION:
        ...
        def generate_login(full_name: FullName, ask_for_remove_inactive_user_if_login_is_exists: bool = True) -> str:
            ...
            def show_user_which_login_is_exists_and_return_user_if_it_inactive(login_string: str) -> User:
                ...
                PR.alert(
                    f"Логин '{login_string}' занят ({user.name} - {'активный' if is_active else 'неактивный'})")
...
class PR:
    ...
    @staticmethod
    def text_color(color: int, string: str) -> str:
    ...
    def input(caption: str) -> None:
        PR.write_line(PR.input_str(caption, after_text=":"))

    @staticmethod
    def input_str(caption: str, before_text: str = TEXT_BEFORE, after_text: str = TEXT_AFTER) -> str:
        return f"{Fore.BLACK}{PR.white_str(caption, before_text, after_text)}{Fore.RESET}"
    ...
    def green_str(string: str, before_text: str = TEXT_BEFORE, after_text: str = TEXT_AFTER) -> str:
        return PR.color_str(Back.LIGHTGREEN_EX, string, before_text, after_text)
    ...
    @staticmethod 
    def newline(color: str = Back.LIGHTBLUE_EX, char: str = " ", width: int = 80) -> None:
        PR.write_line("") if color is None else PR.color(color, char*width)

    @staticmethod
    def _line() -> None:
        PR.empty_line()
        PR.newline(Back.WHITE, PR.text_color(Fore.BLACK, "-"), width=128)
        PR.empty_line()
