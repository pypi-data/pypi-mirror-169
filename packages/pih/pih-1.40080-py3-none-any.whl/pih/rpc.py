...
from pih.collection import ServiceRoleBaseValue, ServiceInformation, ServiceRoleValue
...
class RPC:
    ...
    class UnaryService(pb2_grpc.UnaryServicer):
        ...
        def internal_handler(self, command_name: str, parameters: str, context) -> dict:
            ...
            if command == ServiceCommands.ping:
                ...
                service_role_info["pih_version"] = RPC.PIH_VERSION
                service_role_info["pid"] = RPC.PID
                del service_role_info["libs"]
    ...
    class Service:
        ...
        def serve_role(role: ServiceRoles, handler: Callable, start_handler: Callable = None, test: bool = False) -> None:
            from pih.pih import PIH, PR
            PR.init()
            RPC.PIH_VERSION = PIH.VERSION.local()
    ...
    @staticmethod
    def ping(role: ServiceRoles) -> ServiceInformation:
        try:
            return DataTools.fill_data_from_rpc_str(ServiceInformation(), RPC.internal_call(role, ServiceCommands.ping))
        except Error:
            return False
