...
class ServiceRoles(Enum):
    ...
    DOCS: ServiceRoleValue = ServiceRoleValue("Docs", "Documents api service", None, CONST.RPC.PORT(1),
...
class LogCommands(Enum):
    ...
    SERVICE_STARTED: LogCommand = LogCommand(
        "Сервис {service_name} ({service_description}) запущен!\nИмя хоста: {host_name}\nПорт: {port}\nИдентификатор процесса: {pid}\nИдентификатор родительского процесса: {ppid}", LogChannels.SYSTEM, LogLevels.NORMAL.value, (ParamItem("service_name", "Name of service"), ParamItem("service_description", "Description of service"), ParamItem("host_name", "Name of host"), ParamItem("port", "Port"), ParamItem("pid", "PID"), ParamItem("ppid", "PPID"))),
    #
