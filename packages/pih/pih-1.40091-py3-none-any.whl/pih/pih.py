...
class PIH:
    ...
    def global_except_hook(exctype, value, traceback):
        value_str: str = "\n".join(value.args)
        PIH.MESSAGE.to_debug_bot(value_str, LogLevels.ERROR)
    ...
    class VERSION:
        ...
        def need_update() -> bool:
            #importlib.util.find_spec("pih") is not None and
            return importlib.util.find_spec("pih") is not None and PIH.VERSION.local() < PIH.VERSION.remote()
    ...
    class UPDATER:
        ...
        @staticmethod
        def update_for_role(value: ServiceRoles, show_result: bool = False) -> bool:
            ...
            remote_executor_command_list: List[str] = PIH.PSTOOLS.create_remote_executor_for_service_role_value(
                service_role_value, True)
            command_list: List[str] = remote_executor_command_list + \
                PIH.UPDATER.get_updater_command_list()
            if show_result:
                process_result = subprocess.run(command_list, text=True)
            else:
                process_result = subprocess.run(command_list, stdout=DEVNULL, stderr=STDOUT, text=True)
            returncode = process_result.returncode
            return returncode == 0

        @staticmethod
        def get_updater_command_list() -> List[str]:
            return ["-m", CONST.PYTHON.PYPI, "install", f"{PIH.NAME}=={PIH.VERSION.remote()}"]

        @staticmethod
        def update_localy(show_result: bool = False) -> bool:
            command_list = PIH.UPDATER.get_updater_command_list()
            command_list.pop(0)
            if show_result:
                process_result = subprocess.run(command_list, text=True)
            else:
        ...
        @staticmethod
        def update_remote(host: str, show_result: bool = False) -> bool:
            remote_executor_command_list: List[str] = PIH.PSTOOLS.create_remote_executor(host, True)
            command_list: List[str] = remote_executor_command_list  + PIH.UPDATER.get_updater_command_list()
            if show_result:
                process_result = subprocess.run(command_list, text=True)
            else:
                process_result = subprocess.run(
        ...
        def update_action(start_handler: Callable, update_start_handler: Callable, update_complete_handler: Callable) -> None:
            ...
            def internal_update_action(need_update: bool, start_handler: Callable, update_start_handler: Callable, update_complete_handler: Callable):
                if need_update:
                    ...
                    start_handler()
            Thread(target=internal_update_action, args=(need_update, start_handler, update_start_handler, update_complete_handler,)).start() 
    ...
    class PSTOOLS:
        ...
        @staticmethod
        def create_remote_executor_for_service_role_value(service_role_value: ServiceRoleValue, interactive: bool = False) -> List[str]:
            return PIH.PSTOOLS.create_remote_executor(service_role_value.host, interactive)
    ...
    class SERVICE:
        ...
        class ADMIN:
            ...
            def start(role: ServiceRoles, check_if_started: bool = True, test: bool = False) -> bool:
                ...
                process_result = subprocess.run(remote_executor_command_list, stdout=DEVNULL,
            ...
            def stop(role: ServiceRoles, check_if_started: bool = True, test: bool = False) -> bool:
                ...
                process_result: CompletedProcess = subprocess.run(
                    [ps_kill_executor_path, host, str(service_role_value.pid)], stdout=DEVNULL,
                    stderr=STDOUT)
                returncode = process_result.returncode
        ...
        def check_availability(role: ServiceRoles) -> bool:
            return PIH.SERVICE.ping(role) != False
    ...
    class SESSION:
        ...
        @staticmethod
        def app_restart(file: str) -> None:
            os.execl(sys.executable, 'python', file, *sys.argv)
    ...
    class CHECK:
        ...
        class SERVICE:
            ...
            class ADMIN:
                ...
                @staticmethod
                def availiable() -> bool:
                    return PIH.CHECK.RIGTH.by_group(CONST.AD.Groups.Admin)    
