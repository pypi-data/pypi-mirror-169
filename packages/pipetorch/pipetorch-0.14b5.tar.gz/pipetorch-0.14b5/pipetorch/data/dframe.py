...
class _DFrame:
    ...
    _config_indices = [
                 ...
                 '_pt_train_valid_indices', '_pt_test_indices', '_pt_folds', '_cached_index' ]
    ...
    def test(self):
        ...
        if self._pt_sequence_window is None:
            r = DSet.df_to_testset(self.loc[self._test_indices], self, self._test_indices, self._transforms(train=False))
        else:
            low, high = min(self._test_indices), max(self._test_indices) + self._sequence_window + self._shift_y - 1
            r = DSet.df_to_testset(self.loc[low:high], self, list(range(low, high)), self._transforms(train=False))
    ...
    def train_transforms(self, *transforms):
        ...
        """
        r = self.copy(deep=False)
        r._change('_pt_train_transforms', transforms)
        return r
    ...
    def transforms(self, *transforms):
        ...
        """
        r = self.copy(deep=False)
        r._change('_pt_transforms', transforms)
        return r
