...
@LastEditTime: 2022-09-30 09:40:55
...
class WeChatPayV3Request(object):
    ...
    def transfer_one(self, openid, out_batch_no, batch_name, batch_remark, total_amount):
        """
        :description:  单次商家转账
        :param openid：openid是微信用户在公众号appid下的唯一用户标识（appid不同，则获取到的openid就不同），可用于永久标记一个用户
    ...
    def query_batches(self, batch_id, need_query_detail=False, offset=0, limit=20, detail_status=None):
        ...
        url_path = url_path + "?" + WeChatHelper.key_value_url(data)
        url = self.base_url + url_path
        data = None
        try:
