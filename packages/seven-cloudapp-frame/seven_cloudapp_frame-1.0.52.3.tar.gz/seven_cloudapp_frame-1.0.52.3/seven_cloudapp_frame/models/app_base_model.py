...
@LastEditTime: 2022-09-27 09:09:42
...
class AppBaseModel():
    ...
    def _delete_dict_info_dependency_key(self, parent_id, delay_delete_time=0.01):
        ...
        :last_editors: HuangJianYi
        """
        CmsDictInfoModel().delete_dependency_key(DependencyKey.dict_info_list(parent_id), delay_delete_time)
    ...
    def get_dict_info_list(self, parent_id, page_size, page_index, order_by="id desc",field="*", is_cache=True):
        ...
        dict_info_model = DictInfoModel(context=self.context)
        if is_cache == True:
            page_list, total = dict_info_model.get_cache_dict_page_list(field=field, page_index=page_index, page_size=page_size, where=condition, group_by="", order_by=order_by,params=params,dependency_key=DependencyKey.dict_info_list(parent_id),cache_expire=600)
        else:
            page_list, total = dict_info_model.get_dict_page_list(field=field, page_index=page_index, page_size=page_size, where=condition, group_by="", order_by=order_by,params=params)
        return page_list, total