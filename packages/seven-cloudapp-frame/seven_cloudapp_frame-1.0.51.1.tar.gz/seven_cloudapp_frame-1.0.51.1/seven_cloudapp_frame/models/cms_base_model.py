...
@LastEditTime: 2022-09-26 16:02:14
...
from seven_cloudapp_frame.models.db_models.cms.cms_dict_info_model import *
...
class CmsBaseModel():
    ...
    def _delete_dict_info_dependency_key(self, parent_id, delay_delete_time=0.01):
        """
        ...
        :last_editors: HuangJianYi
        """
        CmsInfoModel().delete_dependency_key(DependencyKey.dict_info_list(parent_id), delay_delete_time)

    def save_dict_info(self, parent_id, dict_id, dict_name, dict_value, dict_pic, sort_index, is_release, i1, i2, s1, s2):
        """
        :description: 保存字典
        :param parent_id: 父节点标识
        ...
        """
        invoke_result_data = InvokeResultData()
        if not parent_id or not dict_name:
            invoke_result_data.success = False
            invoke_result_data.error_code = "param_error"
            invoke_result_data.error_message = "参数不能为空或等于0"
            return invoke_result_data
        old_cms_dict_info = None
        cms_dict_info = None
        is_add = False
        cms_dict_info_model = CmsDictInfoModel(context=self.context)
        if dict_id > 0:
            cms_dict_info = cms_dict_info_model.get_entity_by_id(dict_id)
            if not cms_dict_info:
                invoke_result_data.success = False
                invoke_result_data.error_code = "error"
                invoke_result_data.error_message = "信息不存在"
                return invoke_result_data
            old_cms_dict_info = deepcopy(cms_dict_info)
        if not cms_dict_info:
            is_add = True
            cms_dict_info = CmsDictInfo()
        cms_dict_info.parent_id = parent_id
        cms_dict_info.dict_name = dict_name
        cms_dict_info.dict_value = dict_value
        cms_dict_info.dict_pic = dict_pic
        cms_dict_info.sort_index = sort_index
        cms_dict_info.is_release = is_release
        cms_dict_info.i1 = i1
        cms_dict_info.i2 = i2
        cms_dict_info.s1 = s1
        cms_dict_info.s2 = s2
        cms_dict_info.modify_date = SevenHelper.get_now_datetime()
        if is_add:
            cms_dict_info.create_date = cms_dict_info.modify_date
            cms_dict_info.id = cms_dict_info_model.add_entity(cms_dict_info)
        else:
            cms_dict_info_model.update_entity(cms_dict_info,exclude_field_list="parent_id,create_date")
        result = {}
        result["is_add"] = is_add
        result["new"] = cms_dict_info
        result["old"] = old_cms_dict_info
        invoke_result_data.data = result
        self._delete_dict_info_dependency_key(parent_id)
        return invoke_result_data

    def get_dict_info_list(self, parent_id, page_size, page_index, order_by="id desc",field="*", is_cache=True):
        """
        :description: 字典列表
        :param parent_id：父节点标识
        :param page_size：页大小
        ...
        if not parent_id:
            return page_list, total
        condition = "parent_id=%s and is_release=1"
        params = [parent_id]
        cms_dict_info_model = CmsDictInfoModel(context=self.context)
        if is_cache == True:
            page_list, total = cms_dict_info_model.get_cache_dict_page_list(field=field, page_index=page_index, page_size=page_size, where=condition, group_by="", order_by=order_by,params=params,dependency_key=DependencyKey.dict_info_list(parent_id),cache_expire=600)
        else:
            page_list, total = cms_dict_info_model.get_dict_page_list(field=field, page_index=page_index, page_size=page_size, where=condition, group_by="", order_by=order_by,params=params)
        return page_list, total