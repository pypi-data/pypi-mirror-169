...
@LastEditTime: 2022-09-27 20:49:14
...
class WeChatPayRequest(object):
    """
    :description: 微信支付请求类,配置文件内容 "wechat_pay": {"api_key": "","mch_id": "","certificate_url": "","certificate_key_url": ""}
    """
    """配置账号信息"""
    # =======【基本信息设置】=====================================
    ...
    def __init__(self, app_id="", api_key="", mch_id="", certificate_url="", certificate_key_url=""):
        ...
        self.certificate_key_url = certificate_key_url if certificate_key_url else pay_config.get("certificate_key_url","")
    ...
    def send_red_pack(self, mch_billno, send_name, re_openid, total_amount, total_num, wishing, client_ip, act_name, remark, scene_id=''):
        ...
        try:
            ...
            # 重点是`cert=(certificate_url, certificate_key_url), verify=True`参数，查看源码后才知道是这样传ca证书
            respone = requests.post(requset_url, data=request_xml, cert=(self.certificate_url, self.certificate_key_url), verify=True)
            response_data = xmltodict.parse(respone.text.encode('ISO-8859-1').decode('utf-8'))['xml']
            if response_data['return_code'] == 'SUCCESS':
                if response_data['result_code'] == 'SUCCESS':
