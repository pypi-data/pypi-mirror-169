...
from tecton_proto.data.state_update_pb2 import FcoUpdateTransitionSideEffectStreamRestartType
...
class PlanDiff:
    ...
    def print(self, debug: bool, recreates_suppressed: bool):
        ...
        if recreates_suppressed:
            ...
            for item in self.diffs:
                ...
                if transition_type == "UPDATE" and item.HasField("update_type_side_effects"):
    ...
    def _pprint_plan_item_with_warnings(
        ...
        if plan_item_type == "CREATE":
            ...
            if item.HasField("update_type_side_effects"):
    ...
    def _update_side_effects_transition_display(self, item: state_update_pb2.FcoDiff) -> str:
        stream_restart_type = FcoUpdateTransitionSideEffectStreamRestartType.Name(
            item.update_type_side_effects.stream_restart_type
        )
        if stream_restart_type == "RESTART_STREAM_REUSE_CHECKPOINTS":
            ...
            assert False, f"Unknown Update side effect {item.update_type_side_effects.stream_restart_type}"
