...
class GRPCclient(object):
    def __init__(self, channel) -> None:
        self.user_permission_stub = user_pb2_grpc.UserPermissionControllerStub(channel)
        self.user_stub = user_pb2_grpc.UserControllerStub(channel)
...
class BaseUserServiceTestCase(RPCTransactionTestCase):
    ...
    def setUp(self):
        ...
        super().setUp()
        self.client = GRPCclient(self.channel)
...
class RetrieveUserPermissionTestCase(BaseUserServiceTestCase):

    def test_retrieve_permission_with_non_existent_user(self):
        org = Org.objects.first()
        email = "nonexistent@email.com"

        response = self.client.user_permission_retrieve_request(org_uuid=str(org.uuid), user_email=email)
    ...
    def test_retrieve_user_permission(self):
        ...
        with self.assertRaisesMessage(FakeRpcError, f"Org: {self.WRONG_UUID} not found!"):
            self.client.user_permission_retrieve_request(org_uuid=self.WRONG_UUID, user_email=self.WRONG_EMAIL)

        org.administrators.add(user)

        response = self.client.user_permission_retrieve_request(org_uuid=str(org.uuid), user_email=user.email)
...
class UpdateUserPermissionTestCase(BaseUserServiceTestCase):

    def test_update_permission_with_non_existent_user(self):
        org = Org.objects.first()
        email = "nonexistent@email.com"

        response = self.client.user_permission_update_request(org_uuid=str(org.uuid),
                                                       user_email=email,
    ...
    def test_user_permission_update(self):
        ...
        with self.assertRaisesMessage(FakeRpcError, "adm is not a valid permission!"):
            self.client.user_permission_update_request(org_uuid=str(org.uuid), user_email=user.email, permission="adm")

        update_response = self.client.user_permission_update_request(
            org_uuid=str(org.uuid), user_email=user.email, permission="administrator"
        )
        retrieve_response = self.client.user_permission_retrieve_request(org_uuid=str(org.uuid), user_email=user.email)
        ...
        self.client.user_permission_update_request(org_uuid=str(org.uuid), user_email=user.email, permission="viewer")
        retrieve_response = self.client.user_permission_retrieve_request(org_uuid=str(org.uuid), user_email=user.email)

        self.assertTrue(retrieve_response.viewer)
        self.assertTrue(self.permission_is_unique_true(retrieve_response, "viewer"))

        self.client.user_permission_update_request(org_uuid=str(org.uuid), user_email=user.email, permission="editor")
        retrieve_response = self.client.user_permission_retrieve_request(org_uuid=str(org.uuid), user_email=user.email)

        self.assertTrue(retrieve_response.editor)
        self.assertTrue(self.permission_is_unique_true(retrieve_response, "editor"))

        self.client.user_permission_update_request(org_uuid=str(org.uuid), user_email=user.email, permission="surveyor")
        retrieve_response = self.client.user_permission_retrieve_request(org_uuid=str(org.uuid), user_email=user.email)
...
class RemoveUserPermissionTestCase(BaseUserServiceTestCase):
    ...
    def test_remove_user_permission(self):
        ...
        with self.assertRaisesMessage(FakeRpcError, "adm is not a valid permission!"):
            self.client.user_permission_remove_request(org_uuid=str(org.uuid), user_email=user.email, permission="adm")

        org.viewers.add(user)

        self.client.user_permission_remove_request(org_uuid=str(org.uuid), user_email=user.email, permission="viewer")

        self.assertFalse(org.viewers.filter(pk=user.pk).exists())


class UserServiceTest(BaseUserServiceTestCase):
