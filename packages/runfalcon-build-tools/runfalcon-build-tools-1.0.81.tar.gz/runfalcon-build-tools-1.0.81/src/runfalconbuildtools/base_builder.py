...
class BaseBuilder(ABC):
    ...
    def __run_build__(self):
        ...
        if self.executor.return_code != 0 or self.__is_error_output__(self.executor.stdout) >= 0 :
            ...
            if output_error != None:
                ...
                raise Exception('Error running build script [2].\nCODE:{code}\nERROR\n{error}\nOUTPUT\n{output}'.format( \
                    ...
                    output = self.executor.stdout[len(self.executor.stdout) - 1000:len(self.executor.stdout)] if self.executor.stdout != None else 'None'\
                ))
