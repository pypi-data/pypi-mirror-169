...
from .yocto import (
    find_elf_files_from_image,
    find_elf_files_from_non_native_packages,
    find_symbol_files_to_upload,
    tmp_path_from_image,
)
...
@click.option(
    ...
    required=True,
    help="""
    The path to the image .tar/.tar.bz2/.tar.gz/.tar.xz as produced by Yocto's do_image_*.
    The file is expected to reside in the location where Yocto produced the file
    (tmp/deploy/images/${PACKAGE_ARCH}/images). The tool will locate the symbol files to
    upload in the tmp/work/ directory, based on the binaries that are inside the image.
    """,
    type=click.Path(exists=True, resolve_path=True, dir_okay=False, path_type=pathlib.Path),
)
@click_option_concurrency
@pass_memfault_cli_context
def upload_yocto_symbols(ctx: MemfaultCliClickContext, image: pathlib.Path, **kwargs):
    ...
    Example Yocto Symbol Upload:
        ...
        $ memfault --org-token $ORG_TOKEN \\
            ...
            upload-yocto-symbols --image build/tmp/deploy/images/raspberrypi3/my-image-raspberrypi3.rootfs.tar.bz2
    """
    ctx.obj.update(**kwargs)
    ...
    def _image_elfs(image_tar: pathlib.Path) -> Iterable[ELFFileInfo]:
        with click.progressbar(
            ...
            label="Finding binaries in image...",
        ) as bar:
            yield from bar

    def _tmp_work_elfs(image_tar: pathlib.Path) -> Iterable[ELFFileInfo]:
        with click.progressbar(
            find_elf_files_from_non_native_packages(tmp_path_from_image(image_tar)),
            label="Searching for symbols in tmp/work...",
        ) as bar:
            yield from bar

    result = find_symbol_files_to_upload(
        image_elfs=_image_elfs(image),
        tmp_work_elfs=_tmp_work_elfs(image),
    )
    for missing_elf_info in sorted(result.missing, key=lambda i: i.path):
        log_func = LOG.debug if missing_elf_info.path.suffix in [".ko"] else LOG.warning
        log_func(
            "Could not find symbol file for %s (GNU Build ID: %s)",
            missing_elf_info.path,
            missing_elf_info.gnu_build_id,
        )
    ...
    ElfSymbolDirectoryUploader(
        ...
        file_paths=(elf_info.path for elf_info in result.found),
    ).upload()
