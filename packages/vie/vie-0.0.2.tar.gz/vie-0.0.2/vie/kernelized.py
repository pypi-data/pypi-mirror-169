...
class RandomFourierFeatures(v1.keras.layers.Layer):
  ...
  model.add(v1.keras.layers.Dense(units=num_classes, activation='softmax')
  model.compile(
    loss=v1.keras.losses.categorical_crossentropy, optimizer=..., metrics=...)
  ```
  ...
  def build(self, input_shape):
    input_shape = v1.TensorShape(input_shape)
    ...
    self.input_spec = v1.keras.layers.InputSpec(
    ...
    self.bias = self.add_weight(
        ...
        dtype=v1.float64,
        initializer=v1.random_uniform_initializer(
            minval=0.0,
            maxval=2 * np.pi,
            dtype=v1.float64,
            seed=self.seed),
    ...
    scale = self.add_weight(
        ...
        shape=(1,),
        dtype=v1.float64,
        initializer=v1.constant_initializer(self.scale),
        trainable=True,
        constraint='NonNeg')
  ...
  def call(self, inputs):
    inputs = v1.convert_to_tensor(inputs, dtype=self.dtype)
    inputs = v1.cast(inputs, v1.float64)
    outputs = v1.linalg.matmul(inputs, self.kernel)
    outputs = v1.nn.bias_add(outputs, self.bias)
    return v1.math.cos(outputs)
  ...
  def get_config(self):
    ...
    if isinstance(self.kernel_initializer, init_ops.Initializer):
      kernel_initializer = v1.keras.initializers.serialize(self.kernel_initializer)
    config = {
...
def _get_random_features_initializer(initializer, shape, seed):
  ...
  if isinstance(initializer, six.string_types):
    if initializer.lower() == 'gaussian':
      random_features_initializer = v1.random_normal_initializer(
          stddev=1.0, seed=seed)
    elif initializer.lower() == 'laplacian':
      random_features_initializer = v1.constant_initializer(
