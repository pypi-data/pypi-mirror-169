...
def racplot(rac,output):
    import matplotlib.pyplot as plt
    extended_plot = rac.attrs["hlm_mode"] is not None
    fig = plt.figure(figsize=(10, 5+5*extended_plot),dpi=100)
    gs = fig.add_gridspec(3+3*extended_plot, hspace=0.2)
    ax = gs.subplots(sharex=True, sharey=False)
    plot_vec(axes=ax,df=rac.unstack()["EST_RAC"] * 100000,axes_idx=[1,2,0])
    ...
    if extended_plot: # append hlm residuals plot if transformation has been selected 
        ...
        s=f"{rac.attrs['sp3_b']} - {rac.attrs['sp3_b']}" + f" (HLM in {rac.attrs['hlm_mode']})\nResiduals shown are in ECI frame",
        rotation=90, transform=fig.transFigure, fontfamily="monospace")
        fig.add_artist(line)

        plot_vec(axes=ax,df=rac.attrs["hlm"][1].RES.unstack() * 100000,axes_idx=[3,4,5],legend=False)

        hlm = rac.attrs["hlm"][0][0].reshape(-1).tolist()
    ...
    fig.suptitle(rac.attrs['sp3_a'] + " - " + rac.attrs['sp3_b'] + (f" (HLM in {rac.attrs['hlm_mode']})" if extended_plot else ""),y=0.92)
    fig.patch.set_facecolor("w") #white background (non-transparent)
    fig.legend(bbox_to_anchor = (.955,.89),ncol=2,fontsize=8)
    plt.subplots_adjust(right=0.8)

    fig.savefig(f"{rac.attrs['sp3_a'].split('.')[0]}-{rac.attrs['sp3_b'].split('.')[0]}.pdf" if output is None else output)
