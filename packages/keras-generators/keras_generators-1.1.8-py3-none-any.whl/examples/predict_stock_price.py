...
from keras_generators.generators import TimeseriesDataSource, TimeseriesTargetsParams, DataSet, XYBatchGenerator, \
    TargetTimeseriesDataSource, TensorDataSource
...
if __name__ == '__main__':
    ...
    scaled_price_input_ds = price_input_ds.fit_encode([ScaleEncoder(StandardScaler())])

    next_price_ds = scaled_price_input_ds.get_targets(target_name=_mp.target_name)
    last_price_na = price_input_ds.select_features([_mp.target_symbol_idx])[:][:,-1]
    last_price_ds = TensorDataSource(name=_mp.target_name, tensors=last_price_na)

    targets_ds = last_price_ds+next_price_ds

    # targets_ds = TargetTimeseriesDataSource.from_timeseries_datasource(scaled_price_input_ds, name=_mp.target_name)

    dataset = DataSet(input_sources={_mp.seq_input_name: scaled_price_input_ds}, target_sources={_mp.target_name: targets_ds})
