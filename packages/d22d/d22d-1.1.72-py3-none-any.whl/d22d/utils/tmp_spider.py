...
get_html_www_foxnews_com__us = pass_ex(if_ex_return='')(get_html_www_foxnews_com__us)


def task(url):
    pq_html = pyquery.PyQuery(get_html_www_foxnews_com__us(url=url, _use_cache=USE_CACHE) or '<></>')('.article.story-1')
    
    for item in pq_html:
        logger_debug(dir(item))
