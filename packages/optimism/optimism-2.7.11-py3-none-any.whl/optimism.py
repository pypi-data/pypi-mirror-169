...
- Version 2.7.11 fixes a typo in error messages for bad types when
  creating Constants and Literals. It also fixes a bug in `Literal` that
  prevented recognition of literals when they contained variables.
            ...
            if isinstance(self.types, tuple):
                for typ in self.types:
                    if typ not in allowed:
                        raise TypeError(
                            f"Type {typ} is not a type that a"
                ...
                if self.types not in allowed:
                    raise TypeError(
                        f"Type {self.types} is not a type that a"
                        f" Constant can be (did you mean to use a"
                        f" Literal instead?)."
                    )
