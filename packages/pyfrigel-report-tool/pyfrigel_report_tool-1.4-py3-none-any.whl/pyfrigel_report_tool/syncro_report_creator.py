    ...
    def __add_performance_overview_data(self):
        ...
        for key in list_key:
            ...
            list_molds.append([mold_name, '{} h {} min'.format(int(self.molds_info[key]['on']) if type(self.molds_info[key]['on']) == float else self.molds_info[key]['on'], int((self.molds_info[key]['on'] - int(self.molds_info[key]['on']))*60) if type(self.molds_info[key]['on']) == float else self.molds_info[key]['on'] ), 
                            ...
                            '{} {}'.format(round(self.molds_info[key]['energy_consumption_standard']/self.molds_info[key]['material_produced_standard']*1000) if self.molds_info[key]['material_produced_standard']> 0 else '-', 'Wh/kg'),
                            '{} {}'.format(round(self.molds_info[key]['energy_consumption_syncro']/self.molds_info[key]['material_produced_syncro']*1000) if self.molds_info[key]['material_produced_syncro']> 0 else '-', 'Wh/kg')])
