...
class FinXRest:
    ...
    async def prepare_vol_surface_inputs(self, filename, timeslice_datestamp, timeslice_width_seconds, underlying_symbol) -> str:
        ...
        df_filtered = df.loc[df['years_to_expiry'] > 0.0]
        df_filtered.loc[:, 'strike_price'] = df_filtered.apply(lambda row: self._parse_strike_price(row), axis=1)
        output_df = df_filtered[['years_to_expiry', 'strike_price', 'mark_iv']]
        output_df = output_df.rename(columns={"years_to_expiry": "expiration", "strike_price": "strike", "mark_iv": "price"})
