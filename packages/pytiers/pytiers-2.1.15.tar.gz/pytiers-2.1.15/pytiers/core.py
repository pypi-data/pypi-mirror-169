...
class Tier(object):
    ...
    def add_point(self, point):
        ...
        point = Point(point.time, point.value, superior_Tier=self)
    ...
    def to_plot(self, start_time=None, end_time=None):
        ...
        if start_time >= end_time:
            raise Exception('start_time is later than or equals to end_time.')
    ...
    def get_point(self, by_target, by, strict=True, point_type=None):
        """Get Point of the Tier with chosen method. \nIf `strict` set to False, get the closest Point object to the target. If `point_type` set to \'later\', find the closest later Point; If `point_type` set to \'earlier\', find the closest earlier Point.\nSee `help(by)` for more information on the available methods. To change related parameters, use `parameters`."""
        ...
        if by == _by.TIME:
            if strict:
                if point_type!=None:
                    warnings.warn('`point_type` is not used when `strict` is set to True.')
                for point in self.__points:
                ...
                if point_type == None:
                    pass
                elif point_type == 'later':
                    points = [point for point in points if point.time >= by_target]
                elif point_type == 'earlier':
                    points = [point for point in points if point.time <= by_target]
                else:
                    raise Exception(
                        '`point_type` can only by one of {\'later\', \'earlier\', None}.'
                    )

                points.sort(key=lambda point: abs(point.time - by_target))

                return points[0]

        elif by == _by.INDEX:
            if point_type!=None:
                warnings.warn('`point_type` is not used when by method is not `TIME`.')
        ...
        if not parameters['points.ignore_missing_point']:
            if by==_by.TIME:
                raise Exception('Point not found. Consider setting `strict` to False.')
    ...
    def interpolate_points(self, by_target, by, step=.01, strict=True):
        """Interpolate values between two Point objects (array-like), with given step (by default 0.01s). See `help(by)` for more information on the available methods. To change related parameters, use `parameters`."""

        if by not in _by.by_methods:
            raise Exception(
                'Invalid by_method. Use help() to see valid methods.')

        if len(by_target) != 2:
            raise Exception(
                '`interpolate_points()` takes exactly 2 targets.'
            )

        original_ignore_missing_point_parameter = parameters['points.ignore_missing_point']
        parameters['points.ignore_missing_point'] = False

        if by == _by.TIME and not strict:
            points = self.get_point(min(by_target), by, strict=strict, point_type='earlier'), self.get_point(max(by_target), by, strict=strict, point_type='later')

        else:
            points = self.get_point(
                by_target[0], by), self.get_point(by_target[1], by)
        ...
        self.remove_points((point1.time, point2.time), _by.TIME)

        interpolated_point_times = np.arange(
            point1.time, point2.time, step)[1:]

        interpolated_points = [Point(time, value, superior_Tier=self) for time, value in zip(
            interpolated_point_times, np.interp(interpolated_point_times, [point1.time, point2.time], [point1.value, point2.value]))]

        self.add_points(interpolated_points)

        self._indexed = False
        self._reindex()
        self._indexed = True

        parameters['points.ignore_missing_point'] = original_ignore_missing_point_parameter
    ...
    def _add_point_from_file(self, point):
        ...
        point = Point(point.time, point.value,
                      point.point_index, superior_Tier=self)
...
class Point(object):
    ...
    __slots__ = ['__time', '__value', '__point_index', '__superior_Tier']

    def __init__(self, time, value, point_index=None, superior_Tier=None):
        ...
        self.__superior_Tier = superior_Tier
    ...
    def time(self, time):
        ...
        if self.superior_Tier != None:
            self.superior_Tier._reindex()
    ...
    def point_index(self, point_index):
        ...
        elif not self.superior_Tier.indexed:
            self.__point_index = point_index
    ...
    @property
    def superior_Tier(self):
        """The PitchTier to which this Point object belongs."""

        return self.__superior_Tier
