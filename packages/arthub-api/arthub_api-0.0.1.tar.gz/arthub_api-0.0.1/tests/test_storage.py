...
def test_transfer(tmp_path):
    # download
    res = storage.download_by_path(asset_hub="trial", remote_node_path="sdk_test/storage_test/download",
                                   local_dir_path=str(tmp_path), same_name_override=False)
    if not res.is_succeeded():
        on_operation_failed("download", res.error_message())
        assert 0
    local_downloaded_path = res.data[0]
    logging.info("[TEST][STORAGE] download success, local downloaded path: %s" % local_downloaded_path)

    # upload downloaded dir
    res = storage.upload_to_directory_by_path(asset_hub="trial", remote_dir_path="sdk_test/storage_test/upload",
    ...
    if not res.is_succeeded():
        ...
        assert 0
    logging.info("[TEST][STORAGE] upload success, remote uploaded id: %d" % res.data[0])
