...
class APIResponse(object):
    ...
    def is_api_authentication_failed(self):
        r"""Authentication failed, need to set the correct token by API::set_token().
        """
...
class OpenAPI(object):
    ...
    def depot_get_node_brief_by_ids(self, asset_hub, ids, simplified_meta=False):
        ...
        metas = ["name", "file_format", "node_type", "permission_mask", "direct_child_count"] if simplified_meta \
            else ["name", "node_type", "full_path_id", "full_path_name", "capacity", "file_format",
                  "confirmed_asset_id", "preview_url", "permission_mask", "parent_id", "status", "crc64",
                  "direct_child_count"]
        req_payload = {
            "ids": ids,
            "meta": metas
    ...
    def depot_get_node_brief_by_path(self, asset_hub, root_id, path, simplified_meta=False):
        ...
        metas = ["name", "file_format", "node_type"] if simplified_meta \
            else ["name", "node_type", "full_path_id", "full_path_name", "capacity", "file_format",
                  "confirmed_asset_id", "preview_url", "permission_mask", "parent_id", "status", "crc64",
                  "direct_child_count"]
