...
class RandomForest(TreeEnsembles):
  ...
  def predict_instance(self, instance):
    ...
    for tree in self.forest:
      n_votes[tree.take_decisions_instance(instance)] += 1
  ...
  def to_CNF(self, instance, implicant, target_prediction=None, *, tree_encoding=Encoding.COMPLEMENTARY, cardinality_encoding=Encoding.SEQUENTIAL_COUNTER):
    ...
    new_variables_atleast = [v for v in range(1+n_original_variables,1+n_original_variables+self.n_trees)] # Each tree is represented by a new variable
    ...
    atleast_clauses = CardEnc.atleast(lits=new_variables_atleast, encoding=EncType.seqcounter, bound=condition_atleast, top_id=new_variables_atleast[-1]).clauses
