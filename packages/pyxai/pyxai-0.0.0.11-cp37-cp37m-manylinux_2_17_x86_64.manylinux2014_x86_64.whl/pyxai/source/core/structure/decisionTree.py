...
from pyxai.source.core.structure.type import TypeLeaf, Encoding
...
class DecisionTree(BinaryMapping):
  ...
  def to_tuples(self, node, for_cpp=False):
    ...
    """
    output = [str(node)] if for_cpp is False else [self.get_id_variable(node)]
  ...
  def simplify(self):
    while self._simplify(self.root) is True:
      pass
  ...
  def compute_id_binaries(self, force_features_equal_to_binaries = False):
    ...
    for node in self.nodes:
      if (node.id_feature, node.threshold) not in map_features_to_id_binaries:
        if force_features_equal_to_binaries == False:
          map_features_to_id_binaries[(node.id_feature, node.threshold)] = [id_binary, 1, None]
          map_id_binaries_to_features.append((node.id_feature, node.threshold))
          id_binary += 1
        else:
          map_features_to_id_binaries[(node.id_feature, node.threshold)] = [node.id_feature, 1, None]
          map_id_binaries_to_features[node.id_feature] = (node.id_feature, node.threshold)
      else:
  ...
  def take_decisions_implicant(self, implicant, map_id_binaries_to_features):
    """
