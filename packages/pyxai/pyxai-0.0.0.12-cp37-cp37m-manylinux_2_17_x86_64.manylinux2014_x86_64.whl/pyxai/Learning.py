...
def load(model_directory):
  solver = MLSolver(None)
  files = solver.load_get_files(model_directory)
  solver_names = []
  ...
  if solver_names[0] == Generic().get_solver_name():
    ...
    assert False, "Bad solver names in the directory: " + model_directory

  models = solver.load(model_directory=model_directory)
