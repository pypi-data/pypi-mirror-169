...
class DiscordUserMapping:
    ...
    def detect_tags_user(data_object: dict, user_id_list: list,
        ...
        """
        if include_author:
            if data_object.get('author', {}).get('id', 'NA') in user_id_list:
                return 1
        if include_mentioned:
            mentioned_users = data_object.get('mentions', [])
    ...
    def __call__(self, data_object: dict, user_id_list: list,
        ...
        return DiscordUserMapping.detect_tags_user(data_object, user_id_list,
                                                   include_author,
                                                   include_mentioned)
