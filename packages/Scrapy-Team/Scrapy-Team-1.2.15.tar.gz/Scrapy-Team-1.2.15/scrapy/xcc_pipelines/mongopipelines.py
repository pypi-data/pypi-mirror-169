...
class MongodbPipeline(object):
    ...
    def _process_item(self, item, spider):
        ...
        if postItem.get("all_json", None):  # all_json :: json
            if isinstance(postItem["all_json"], dict):
                postItem["all_json"] = json.dumps(dict(postItem["all_json"]),ensure_ascii=False)
        if postItem.get("raw_other_pdf_url", None):  # raw_other_pdf_url :: json
            if isinstance(postItem["raw_other_pdf_url"], dict):
                postItem["raw_other_pdf_url"] = json.dumps(dict(postItem["raw_other_pdf_url"]),ensure_ascii=False)
        if postItem.get("other_pdf_url", None):  # other_pdf_url :: json
            if isinstance(postItem["other_pdf_url"], dict):
