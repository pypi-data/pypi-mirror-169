...
def transform(path,fname,title,artist,saved_path='./',deleted=False):
    ...
    plt.savefig(os.path.join(saved_path,fname+'.jpg'),format='jpg')
...
if __name__=="__main__":
    ...
    titles=['몽환의 숲','좋은날','예술이야']
    artist=[['키네틱플로우'],'아이유','싸이']
    MUSIC_DIR='./music'
    MEL_DIR='./mel'
    for title,artist in zip(titles,artist):

        url=search(title,artist)
        fname=download(url,MUSIC_DIR)
        transform(MUSIC_DIR,fname,title,artist,MEL_DIR,deleted=True)
