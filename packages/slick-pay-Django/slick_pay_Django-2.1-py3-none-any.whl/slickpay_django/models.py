...
from slick_pay_django.utils import get_webhook_url

from slickpay_lib.utils import extract_redirect_url
from slickpay_lib.invoice import new_invoice
...
class AbstractPayment(models.Model):
    ...
    rib = models.CharField(max_length=25, default=0, blank=True)
    ...
    def get_payment_data(self) -> dict:
        ...
        request_data["fname"] = settings.FNAME
        request_data["lname"] = settings.LNAME
        request_data["amount"] = float(self.amount)
        request_data["address"] = settings.ADDRESS
        request_data["rib"] = settings.RIB
        # request_data["webhook_url"] = self.generate_webhook_url()
        request_data["returnUrl"] = self.returnUrl
