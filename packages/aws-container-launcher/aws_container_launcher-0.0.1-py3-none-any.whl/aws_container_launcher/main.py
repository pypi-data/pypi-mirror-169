...
class ContainerLauncher:
    ...
    def start_container(self, container_input):
        ...
        ec2_sg_id = container_input.get('ec2_sg_id')

        try:
            self.__options['name_suffix'] = entry_id
            ...
            is_spot = container_input.get('is_spot', True)
            ...
            if container_input.get('instance_type'):
                ...
                instance_type = ec2.get_optimal_instance_type({
            ...
            ami = container_input['ami'] if container_input.get('ami') \
                else Ec2().get_image_id({'instance_type': instance_type})
            subnet_ids = container_input['subnet_ids']

            ec2_iam_role_arn = self.__create_ec2_iam_role()
            self.__create_ecs_task_iam_role()
            ...
            self.__create_launch_template({
                ...
                'storage': container_input.get('storage'),
                'is_spot': is_spot,
                'ec2_sg_id': ec2_sg_id
            })
            self.__create_asg({
            ...
            self.__wait_for_instance_deploying()
            ...
            logs = {
                ...
                'log_stream': f'{self.__ECS_TASK_LOG_STREAM_PREFIX}-{entry_id}'
            }
    ...
    def __create_instance_profile(self):
        ...
        # waiter = self.clients['iam'].get_waiter('instance_profile_exists')
    ...
    def __create_ec2_iam_role(self):
        name_suffix = self.__options['name_suffix']
        role_name = f'{self.__name_prefix}-ec2-role-{name_suffix}'
        policy_name = f'{self.__name_prefix}-ec2-policy-{name_suffix}'
        ...
        self.__resources['ec2_iam_policy_name'] = policy_name

        # waiter = self.clients['iam'].get_waiter('role_exists')
        # waiter.wait(
        #    RoleName=role_name,
        # )
    ...
    def __create_ecs_task_iam_role(self):
        name_suffix = self.__options['name_suffix']
        role_name = f'{self.__name_prefix}-ecs-task-role-{name_suffix}'
        policy_name = f'{self.__name_prefix}-ecs-task-policy-{name_suffix}'
        ...
        self.__resources['ecs_task_iam_policy_name'] = policy_name
    ...
    def __destroy(self):
        ...
        try:
            if self.__resources.get('ec2_iam_role_name') and self.__resources.get('ec2_iam_policy_name'):
                self.clients['iam'].delete_role_policy(
                    RoleName=self.__resources['ec2_iam_role_name'],
                    PolicyName=self.__resources['ec2_iam_policy_name']
                )
                self.logger.debug(f"The IAM policy {self.__resources['ec2_iam_policy_name']} has been deleted")
        except Exception as e:
        ...
        try:
            if self.__resources.get('asg_scale_in_alarm_name'):
                ...
                self.logger.debug(f"The Cloudwatch alarm {self.__resources['asg_scale_in_alarm_name']} has been deleted")
        except Exception as e:
        ...
        try:
            if self.__resources.get('capacity_provider_name'):
                ...
                self.logger.debug(f"The capacity provider {self.__resources['capacity_provider_name']} has been deleted")
        except Exception as e:
        ...
        try:
            if self.__resources.get('ecs_task_iam_policy_name'):
                self.clients['iam'].delete_role_policy(RoleName=self.__resources.get('ecs_task_iam_role_name'),
                                                       PolicyName=self.__resources.get('ecs_task_iam_policy_name'))
                self.logger.debug(
    ...
    def __create_asg(self, asg_opts):
        ...
        self.clients['autoscaling'].create_auto_scaling_group(
            AutoScalingGroupName=asg_name,
            # LaunchConfigurationName=self.__resources['launch_config_name'],
            LaunchTemplate={
                'LaunchTemplateName': self.__resources['launch_template_name'],
            },
            MinSize=0,
    ...
    def __create_ecs_task_definition(self, task_def_opts, instance_type):
        ...
        response = self.clients['ecs'].register_task_definition(
            ...
            executionRoleArn=self.__resources['ecs_task_iam_role_name'],
            networkMode='host',
    ...
    def __wait_for_instance_deploying(self):
        ...
        while True:
            ...
            diff = now_time - start_time
            diff_minutes = diff.total_seconds() / 60
            if diff_minutes > self.__EC2_INSTANCE_DEPLOYING_TIMEOUT_IN_MINUTES:
    ...
    def __create_launch_template(self, launch_template_opts):
        ...
        storage = launch_template_opts.get('storage')
        user_data = launch_template_opts.get('user_data', '')
        instance_type = launch_template_opts['instance_type']
        ec2_sg_id = launch_template_opts['ec2_sg_id']
        is_spot = False if launch_template_opts.get('is_spot') is not None and launch_template_opts[
            'is_spot'] == False else True
        ...
        if ec2_sg_id is not None:
            launch_template_data['SecurityGroupIds'] = [ec2_sg_id]
