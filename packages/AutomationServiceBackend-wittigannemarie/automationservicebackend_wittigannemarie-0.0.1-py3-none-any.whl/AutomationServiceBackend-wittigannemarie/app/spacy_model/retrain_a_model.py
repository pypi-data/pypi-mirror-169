...
from app.helper.spacy_trainer import model_helper

class model_retrainer():
    '''
    The class manages all actions that are needed to prepare, move, etc files for retraining a model and trigger the interactions with Spacy 
    '''
    load_dotenv()
    ...
    def uploadfile_not_empty (self, path_to_file, filename):
        if (not self.file_helper.check_file_size(path_to_file)):
            raise NoTrainingdataException(filename + " either couldn't be saved properly or has a size of 0 and therefore is empty. Please make sure to send a full file.")
        else:
            return 

    def save_to_disk(self, upload_file: UploadFile, destination: Path) -> None:
        try:
    ...
    def remove_recursively (self, path_to_dir: str):
        '''
    ...
    def cleanup(self):
        '''
        Move the new model and the training and testing files to the constant directories and remove all leftover temporary files 
        '''
    ...
    def transfer_json_to_csv (self, json_data: json, header: List, path: str):
        '''
        ...
        with open(path, 'w', encoding='UTF8', newline='') as f:
            ...
            for entry in json_data:
                ...
                try:
                    ...
                    raise Exception('The row generated from ' + entry["text"] + ' could not be added. Please check the values. Process continues.') from e
