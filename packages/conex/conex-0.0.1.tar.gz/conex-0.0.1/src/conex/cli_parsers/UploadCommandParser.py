...
def _execute_upload(args, conex_api):
    conex_api.upload(args.path, args.lock, args.remote, args.all)


def setup_command(subparsers, conex_api):
    ...
    parser.add_argument("--all", default=False, action='store_true', help="Upload both package recipe and packages")
