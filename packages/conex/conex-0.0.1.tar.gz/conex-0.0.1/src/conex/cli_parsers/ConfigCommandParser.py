...
def _setup_get_command(subparsers, conex_api):
...
def _set_config(input_: str, conex_api):
    tokens = input_.split("=")
    item = tokens[0]
    value = tokens[1]
...
def _setup_set_command(subparsers, conex_api):
...
def _setup_remove_command(subparsers, conex_api):
