...
def __docker_compose_cli__() -> str:
    """Checks for `docker compose`. If not found, fallback to
    `docker-compose`"""
    try:
        subprocess.check_output("docker compose --help", shell=True, text=True)
        return "docker compose"
    except Exception as e:
        logging.warning(
            f"docker compose plugin is not installed? {e}. Falling back to docker-compose"
        )
        compose = shutil.which("docker-compose")
        assert compose, f"Could not find `docker compose` or `docker-compose`"
        return compose
...
class ComposeFile:
    ...
    def default_script_path(self) -> Path:
        """The default script. It is first script that is executed."""
    ...
    def setup_docker_file(self):
        ...
        self.dockerfile_params[
            "templ_run_steps"
        ] = f"RUN bitia mpm ensure {' '.join(executables)}"
    ...
    def run(self, check: bool = False, recreate: bool = False, **kwargs):
        ...
        docker_compose = __docker_compose_cli__()
        assert docker_compose

        cmds = []
        if recreate:
            cmds += [f"{docker_compose} down"]
            cmds += [f"{docker_compose} build --no-cache"]

        if kwargs.get("versbose", False):
            # Add a command that shows information inside the container.
            pass

        args = "--no-color --abort-on-container-exit --no-log-prefix"
        cmds += [f"{docker_compose} up {args}"]
