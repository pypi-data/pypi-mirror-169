...
class Search():
            ...
            for sol_k, sol_v in sol_i.items():
                ...
                if (hasattr(har_mem_sol[sol_k], 'dtype') and
                   har_mem_sol[sol_k].dtype == 'O'):
                    if (len(har_mem_sol[sol_k]) == len(sol_v) and
                       np.all(np.concatenate(np.equal(har_mem_sol[sol_k],
                                                   sol_v,
                                                   dtype=np.object)))):
    ...
    def run_search_latent(self, HMS=10, min_classes=1, max_classes=10,
        ...
        for i in range(num_classes_considered):
            ...
            all_bic = [sol['bic'] for sol in all_harmony if sol['class_num'] == i]
            all_val = [sol['val'] for sol in all_harmony if sol['class_num'] == i]
        ...
        if self.multi_objective:
            ...
            lns_all = []
            for i in range(num_classes_considered):
                lns = ax.scatter(all_bic_classes[i], np.log(all_val_classes[i]), label=f"All solutions - {i} classes", marker='o')
                lns_all.append(lns)
