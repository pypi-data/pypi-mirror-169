from amqp_client_python import ConnectionRabbitMQ
...
class EventbusRabbitMQ:
    ...
    async def rpc_client(self, exchange: str, routing_key: str, body: str):
        self.rpc_connection.open()
        self.rpc_connection.channel_open()
        return await self.rpc_connection.rpc_client(exchange, routing_key, body)

    def publish(self, event:IntegrationEvent, routing_key: str, ioloop_active = False):
        self.pub_connection.open()
    ...
    def provide_resource(self, name: str, callback, auto_ack=True):
