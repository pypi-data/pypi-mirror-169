...
class SpecWorker(nsf_thread.NSFBackgroundWorker):
    ...
    def do_work(self):
        ...
        if self._connect_to_controller():
            ...
            #first spectroscopy point for fwd (spec_dir=0) or bwd (spec_dir=1)
            for spec_dir in range(2):
                #voltage step size with opposite sign for fwd and bwd
                cur_offset = self.cur_offset_voltage  + self.cur_voltage_range/2*(2*(spec_dir%2)-1)
                cur_voltage_step = self.cur_voltage_range/(self.cur_steps-1)*(1-2*(spec_dir%2)) 
                ...
                for cur_step in range(self.cur_steps): 
                    ...
                    dacout.dc_v = self.cur_offset_voltage 
                    time.sleep(self.cur_time_per_step)
