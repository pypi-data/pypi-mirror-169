...
from __future__ import \
    annotations  # | operator in parse() return value type definition
...
class SuffixNotFoundError(Exception):
    def __init__(self, url):
        self.message = f"Suffix not found in `{url}`"
...
class HTTPURI(URI):
    ...
    def _normalize(self):
        ...
        if parts.scheme:
            ...
            if (
                not "localhost"
                and not re.match(r"[\d.]+", self.host)
                and not self.suffix
            ):
                raise SuffixNotFoundError(self.given)
...
class TelURI(URI):
    ...
    def normalized(self):
        return self.given
        # XXX return "tel:{}".format(self._normalized)
...
class MailtoURI(URI):
    ...
    @property
    def normalized(self):
        return "{}".format(self._normalized)
