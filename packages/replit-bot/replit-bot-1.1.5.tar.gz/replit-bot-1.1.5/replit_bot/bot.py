...
class Bot(Client):
    ...
    def __init__(self,
        ...
        self._call_when_followed = lambda ctx, person_who_followed: logging.info(
            f"{green}[INFO]{end} {person_who_followed} followed bot")
    ...
    def run(self,
        ...
        def on_ready(client):
            logging.info(
                f"{bold_green}[STARTING BOT]{end} Botting \"{bold_blue}{client.user.username}{end}\""
            )
        ...
        def _run(notif) -> None:
            ...
            if (getattr(notif, "comment", False)):
                ...
                self._call_when_followed(notif.comment, notif.creator.username)
