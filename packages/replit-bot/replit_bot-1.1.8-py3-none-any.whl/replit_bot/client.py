    ...
    def fetch(self, userResolvable, options: Dict[str, Any] = {}):
        ...
        if (user_type == str):
            ...
            if (getattr(userResolvable, "username", None)):
                query, variables = ("userByUsername", {
...
class ReplManager:
        ...
        if (getattr(self, "user", False) and self.user):
            if (self.user == self.c.user):
                ...
                for path in options.paths:
                    ...
                    repls = res["currentUser"]["replFolderByPath"]["repls"][
                        "items"]
                    for r in repls:
                        repl = Repl(self.c)
                        repl.update(r)
            ...
            if (typeof.case(str)):
                ...
                if (getattr(replResolvable, "id", False)
                        and replResolvable.id):
                    vars = {"id": replResolvable.id}
            else:
                return None
            if (not options.force and "id" in vars
                    and vars["id"] in self.cache):
    ...
    def likes(self, replResolvable, **options: Dict[str, Any]) -> List[str]:
        ...
        if ("repl" not in res and not res["repl"]): return None
        users = list(
            map(
                lambda post: list(
                    map(lambda vote: vote["user"]["username"], post["votes"][
                        "items"])), res["repl"]["posts"]["items"]))[-1]
        return users
...
class PostManager:
    ...
    def trending(self, options: Dict[str, Any] = {}) -> Dict[str, Any]:
        if (type(self.parent).__name__ == "Client"
...
class FollowingManager:
    ...
    def setFollowing(self,
                     ...
                     options: Dict[str, Any] = {"cache": True}) -> bool:
        user = self.c.users.fetch(userResolvable)
    ...
    def fetch(self, commentResolvable=None, **arguements: Dict[str, Any]):
        ...
        if (name.case("Client") or name.case("Bot")):
            ...
            if (options.cache):
                ...
                if (getattr(comment, "parentComment", None)
                        and comment.parentComment):
                    self.cache[
                        comment.parentComment.id] = comment.parentComment
            return comment
        elif (name.case("Repl")):
...
class User:
    ...
    def update(self, data: Dict[str, Any]):
        ...
        if (timeCreated is not None): self.timeCreated = timeCreated
...
class CurrentUser(User):
    ...
    def update(self, data: Dict[str, Any] = {}):
        ...
        if (login["usernameRepl"]):
            ...
            self.usernameRepl = self.c.repls.fetch(login["usernameRepl"]["id"],
                                                   **login["usernameRepl"])
        return self
    ...
    def change(self, options: Dict[str, Any] = {}):
        ...
        if (image):
            ...
            if (i):
                ...
                data = _raw_post("https://replit.com/data/images/upload",
                                 data={
...
class Repl:
    ...
    def fork(self, options: Dict[str, Any] = {}):
        current = {
            ...
            "description":
            self.description,
            "language": (self.templateInfo.label if getattr(
                self.templateInfo, "label", False) else
    ...
    def update(self, data: Dict[str, Any]):
        ...
        if (notif_data["replComment"]):
            self.comment = self.c.comments.fetch(
                int(notif_data["replComment"]["id"]),
                **notif_data["replComment"])
        return self
...
class Comment:
    ...
    def delete(self) -> None:
        ...
        if (self.currentUserPermission["delete"] or self.isAuthor
                or self.repl.isOwner):
    ...
    def reply(self, body: str, **options: Dict[str, Any]):
        ...
        current.update(options)
        options = JSDict(current)
        send_text = (f"@{self.user.username} {body}" if
                     (getattr(options, "mention", False)
                      and options.mention) else body)
        res = post(
            self.c.sid,
