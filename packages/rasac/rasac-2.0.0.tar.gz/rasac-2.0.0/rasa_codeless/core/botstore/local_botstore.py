...
class LocalBotStore:
    def __init__(
        ...
        self.logdir = logdir
        self.tensorboard_results = TensorBoardResults()
    ...
    def delete_model(self) -> NoReturn:
        raise NotImplementedError("delete_model is not implemented")
            ...
            if isinstance(model_name, Text):
                ...
                if curve:
                    return {
                        "model_id": model_name,
                        "test_acc": self.tensorboard_results.validation_acc(
                            botstore_model=os.path.join(self.botstore_path, model_name)
                        )[0],
                        "train_acc": self.tensorboard_results.training_acc(
                            botstore_model=os.path.join(self.botstore_path, model_name)
                        )[0],
