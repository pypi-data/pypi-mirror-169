# -*- coding: utf-8 -*-
"""
Created on Tue Sep 27 12:31:28 2022
...
def confussion_matrix_pretty(cm, labels):
...
def create_roc_curve(model_predict, test, feature="motion"):
    ...
    # -- Prepare a dataframe, required by our graphinx code
    df_for_graphinx = pandas.DataFrame(dict(threshold=thresholds, fpr=false_positive_rates, tpr=true_positive_rates))
    # -- Generate the plot
    fig = scatter_2D(df_for_graphinx, label_x="fpr", label_y="tpr", x_range=[-0.05,1.05])
    fig.update_traces(mode='lines') # Comment our this line if you would like to see points rather than lines
    fig.update_yaxes(range=[-0.05, 1.05])

    # Return the graph
    return fig, df_for_graphinx
