...
def wait_for_next_stage():    
    parser = argparse.ArgumentParser(description='Wait for next stage')
    parser.add_argument('--during', type=int, default=10, help='how many seconds to wait', required=False)
    args = parser.parse_args()
    print(f"waiting for {args.during} seconds to next stage")
    time.sleep(args.during)
