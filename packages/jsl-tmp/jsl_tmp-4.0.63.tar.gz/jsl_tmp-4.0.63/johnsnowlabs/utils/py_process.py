...
"""
    ...
    suc, proc = execute_py_script_as_new_proc(out_path)
...
def test_list_of_py_script_strings(py_script_paths):
    total = len(py_script_paths)
    df = []
    for i, p in enumerate(py_script_paths):
        print(f'Testing {i}/{total}')
        df.append(execute_py_script_string_as_new_proc(p, file_name=f'{i}_TEST.py'))
