...
#                     return pd.DataFrame(columns=results)
    ...
    async def retrieve_profile_layout_record_type_matrix_by_objects(self, objects_to_retrieve_profile_layout_matrix = ['Account','Address_vod__c','Child_Account_vod__c'], discard_unqueriable_objects = False, return_pivot_table = False):
        ...
        account_page_layouts_bulk['Edit Link'] = account_page_layouts_bulk.apply(lambda row: "https://" + sf_meta_instance + ".lightning.force.com/lightning/setup/ObjectManager/" + row['DeveloperName'] + "/PageLayouts/" + row['ProfileLayout.LayoutId'] + "/view", axis=1)
    ...
    async def get_layout_metadata(self, layout_name):
        ...
        parsed_layout_metadata['layoutSectionsDataFrame'] = parse_layoutSections(layout_metadata['layoutSections'][0]) if len(layout_metadata['layoutSections'][0]) > 0 else pd.DataFrame()
        parsed_layout_metadata['platformActionListDataFrame'] = parse_platformActionList(layout_metadata['platformActionList'][0]) if len(layout_metadata['platformActionList'][0]) > 0 else pd.DataFrame()
        parsed_layout_metadata['quickActionListDataFrame'] = parse_quickActionList(layout_metadata['quickActionList'][0]) if len(layout_metadata['quickActionList'][0]) > 0 else pd.DataFrame()
        parsed_layout_metadata['relatedListDataFrame'] = pd.DataFrame(layout_metadata['relatedLists'][0]) if len(layout_metadata['relatedLists'][0]) > 0 else pd.DataFrame()
