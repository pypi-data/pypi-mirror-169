...
const CONSTANT_MEM ftype psics_data[23] = {
  -.038057080835217922,
   .491415393029387130, 
  -.056815747821244730,
   .008357821225914313,
...
const CONSTANT_MEM cheb_series psi_cs = {
  psics_data,
  22,
  -1, 1,
  17
...
const CONSTANT_MEM ftype apsics_data[16] = {    
  -.0204749044678185,
  -.0101801271534859,
   .0000559718725387,
  -.0000012917176570,
...
const CONSTANT_MEM cheb_series apsi_cs = {
  apsics_data,
  15,
  -1, 1,
  9
...
// const ftype psi_table[PSI_TABLE_NMAX+1] = {
//   0.0,  /* Infinity */              /* psi(0) */
//  -M_EULER,                          /* psi(1) */
...
// const ftype psi_1_table[PSI_1_TABLE_NMAX+1] = {
 ...
 */
static int
psi_x(const ftype x, gsl_sf_result * result)
{
  const ftype y = fabs(x);
  ...
  if(x == 0.0 || x == -1.0 || x == -2.0) {
    ...
    const ftype t = 8.0/(y*y)-1.0;
    gsl_sf_result result_c;
    cheb_eval_e(&apsi_cs, t, &result_c);
    if(x < 0.0) {
      const ftype s = sin(M_PI*x);
      const ftype c = cos(M_PI*x);
    ...
    if(x < -1.0) { /* x = -2 + v */
      const ftype v  = x + 2.0;
      const ftype t1 = 1.0/x;
      const ftype t2 = 1.0/(x+1.0);
      const ftype t3 = 1.0/v;
...
//    */
//   const ftype c1 = -0.1;
//   const ftype c2 =  1.0/21.0;
//   const ftype c3 = -0.05;
//
//   ctype zi = cdiv(C(1,0), z);
//   ctype w  = cmul(zi, zi);
//   ctype cs;
//
//   /* Horner method evaluation of term in parentheses */
//   ctype sum;
//   sum = cmul(w, C(c3/c2,0));
//   sum = cadd(sum, C(1.0,0));
//   sum = cmul(sum, C(c2/c1,0));
//   sum = cmul(sum, w);
//   sum = cadd(sum, 1.0);
//   sum = cmul(sum, C(c1,0));
//   sum = cmul(sum, w);
...
//   cs = cmul(sum, w);
//   cs = cmul(cs, C(-1.0/12.0));
//   cs = cadd(cs, cmul(zi, C(-0.5,0)));
//
//   return ctype_add(ctype_log(z), cs);
// }
//
//
//
...
//   {
//     const ftype sp = sqrt(20.0 + GSL_IMAG(z));
//     const ftype sn = sqrt(20.0 - GSL_IMAG(z));
//     const ftype rhs = sp*sn - GSL_REAL(z);
//     if(rhs > 0.0) n_recurse = ceil(rhs);
//   }
//
//   /* compute asymptotic at the large value z + n_recurse */
//   a = psi_complex_asymp(ctype_add_real(z, n_recurse));
//
...
//   {
//     ctype zn = ctype_add_real(z, i - 1.0);
//     ctype zn_inverse = ctype_inverse(zn);
//     a = ctype_sub(a, zn_inverse);
//
...
//  */
// static int
// psi_n_xg0(const int n, const ftype x, gsl_sf_result * result)
// {
//   if(n == 0) {
...
//     const ftype c2 = -1.0/12.0;
//     const ftype c3 =  1.0/120.0;
//     const ftype c4 = -1.0/252.0;
//     const ftype c5 =  1.0/240.0;
//     const ftype ni2 = (1.0/n)*(1.0/n);
//     const ftype ser = ni2 * (c2 + ni2 * (c3 + ni2 * (c4 + ni2*c5)));
...
// 
// 
int gsl_sf_psi_e(const ftype x, gsl_sf_result * result)
{
  /* CHECK_POINTER(result) */
...
// 
// int
// gsl_sf_psi_1piy_e(const ftype y, gsl_sf_result * result)
// {
//   const ftype ay = fabs(y);
// 
//   /* CHECK_POINTER(result) */
// 
//   if(ay > 1000.0) {
//     /* [Abramowitz+Stegun, 6.3.19] */
//     const ftype yi2 = 1.0/(ay*ay);
//     const ftype lny = log(ay);
//     const ftype sum = yi2 * (1.0/12.0 + 1.0/120.0 * yi2 + 1.0/252.0 * yi2*yi2);
//     result->val = lny + sum;
...
//   }
//   else if(ay > 1.0){
//     const ftype y2 = ay*ay;
//     const ftype x  = (2.0*ay - 11.0)/9.0;
//     const ftype v  = y2*(1.0/(1.0+y2) + 0.5/(4.0+y2));
//     gsl_sf_result result_c;
...
//     const int M = 50;
//     const ftype y2 = y*y;
//     const ftype c0 = 0.00019603999466879846570;
//     const ftype c2 = 3.8426659205114376860e-08;
//     const ftype c4 = 1.0041592839497643554e-11;
//     const ftype c6 = 2.9516743763500191289e-15;
//     const ftype p  = c0 + y2 *(-c2 + y2*(c4 - y2*c6));
//     ftype sum = 0.0;
//     ftype v;
//     
//     int n;
...
//      * ftype-precision for n > 100
//      */
//     const ftype c0 = -1.0/30.0;
//     const ftype c1 =  1.0/42.0;
//     const ftype c2 = -1.0/30.0;
//     const ftype ni2 = (1.0/n)*(1.0/n);
//     const ftype ser =  ni2*ni2 * (c0 + ni2*(c1 + c2*ni2));
...
// 
// 
// int gsl_sf_psi_1_e(const ftype x, gsl_sf_result * result)
// {
//   /* CHECK_POINTER(result) */
// 
//   if(x == 0.0 || x == -1.0 || x == -2.0) {
...
//     const ftype sin_px = sin(M_PI * x);
//     const ftype d = M_PI*M_PI/(sin_px*sin_px);
//     gsl_sf_result r;
...
// 
// 
// int gsl_sf_psi_n_e(const int n, const ftype x, gsl_sf_result * result)
// {
//   /* CHECK_POINTER(result) */
// 
//   if(n == 0)
...
//     ctype z = ctype_rect(x, y);
//     ctype omz = ctype_rect(1.0 - x, -y);
//     ctype zpi = ctype_mul_real(z, M_PI);
//     ctype cotzpi = ctype_cot(zpi);
...
// #include "eval.h"
// 
// ftype gsl_sf_psi_int(const int n)
// {
...
// 
// ftype gsl_sf_psi_1piy(const ftype x)
// {
//   EVAL_RESULT(gsl_sf_psi_1piy_e(x, &result));
// }
// 
// ftype gsl_sf_psi_1_int(const int n)
// {
//   EVAL_RESULT(gsl_sf_psi_1_int_e(n, &result));
// }
// 
// ftype gsl_sf_psi_1(const ftype x)
// {
//   EVAL_RESULT(gsl_sf_psi_1_e(x, &result));
// }
// 
// ftype gsl_sf_psi_n(const int n, const ftype x)
// {
