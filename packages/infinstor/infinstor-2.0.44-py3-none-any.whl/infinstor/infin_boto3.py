...
class ListObjectsPaginator():
    def __init__(self, type2, infin_boto_client, infinsnap_buckets = [],
        ...
        print('All managed buckets: ', self.all_buckets)
...
class InfinBotoClient():
    ...
    def list_objects(self, *args, **kwargs):
        if self.timespec_info['type'] == 'infinsnap' or self.timespec_info['type'] == 'infinslice':
            ...
            if request_bucket in self.timespec_info['infinsnap_buckets']:
                ...
                return self.list_objects_presigned(request_bucket, request_prefix, None)
    ...
    def list_objects_v2(self, *args, **kwargs):
        ...
        if self.timespec_info['type'] == 'infinsnap' or self.timespec_info['type'] == 'infinslice':
            ...
            if request_bucket in self.timespec_info['infinsnap_buckets']:
                ...
                return self.list_objects_v2_presigned(request_bucket, request_prefix, None)
            #elif request_bucket in self.timespec_info['non_infinsnap_buckets']:
            else:
                kwargs['Bucket'] = request_bucket
                kwargs['Prefix'] = request_prefix
    ...
    def download_file(self, *args, **kwargs):
        print_unbufferred('download_file', args, kwargs)
        if self.timespec_info['type'] == 'infinsnap' or self.timespec_info['type'] == 'infinslice':
            ...
            if request_bucket in self.timespec_info['infinsnap_buckets']:
                ...
                if run_id:
                    print_unbufferred("Download file using presigned url")
    ...
    def get_presigned_url(self, bucket, prefix, run_id, method):
        if run_id:
