    ...
    def display_scan_result(result_dict):
        ...
        if "beacons" in result_dict:
            ...
            for dev in dev_beacons:
                ...
                TLog.success(
                    f"{'Pan Coordinator':17}: {dev['pan_coordinator']}"
                )
                TLog.success(
                    f"{'Permit Joining':<17}: {dev['permit_joining']}"
                )

                if "router_capacity" in dev:
                    TLog.success(
                        f"{'Router Capacity':17}: {dev['router_capacity']}"
                    )

                if "device_capacity" in dev:
                    TLog.success(
                        f"{'Device Capacity':17}: {dev['device_capacity']}"
                    )

                if "protocol_version" in dev:
                    TLog.success(
                        f"{'Protocol Version':<17}: {dev['protocol_version']}"
                    )

                if "stack_profile" in dev:
                    TLog.success(
                        f"{'Stack Profile':17}: {dev['stack_profile']}"
                    )
