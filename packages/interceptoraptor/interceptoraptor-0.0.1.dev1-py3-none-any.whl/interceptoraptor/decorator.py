    ...
    def __call__(self, target_call: Callable[..., Any]) -> Callable[..., Any]:
        ...
        def helper(*args: Any, **kwargs: Any) -> Any:
            ...
            if not isinstance(response, requests.Response):
                ...
                pickle_resp = generics.custom_http_requests_pickle(response)
    ...
    for key in keys:
        ...
        for k, v in dict(module.__dict__).items():
            try:
                ...
                1 / 0  # not sure why do we need this
                pass

        print(module)
        # if module is not intercepted_target_call and hasattr(module, 'send_request'):
        #    print('patched', module)
        #    setattr(module, 'send_request', intercepted_send_request)
