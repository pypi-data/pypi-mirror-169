...
class DiscordAPI(RequestsAPI, ClassAPI):
    ...
    def __init__(self, config_path=str(Path(Path.home(), '.config', 'capstone_api')), config_file='capstone_api.ini', config_key="discord"):
        ...
        if capstone_api_ini.exists():
            ...
            RequestsAPI.__init__(self, self.API_URL, self.API_KEY)
    ...
    def getMembers(self, delay=0):
        ...
        if not self.members:
            ...
            self.members = self.get(path, params={"limit": 200})
        return self.members
    ...
    def getStudent(self, delay=0, user_id=0):
        """Get Student Info (by user_id)"""
        if not self.members:
            self.getMembers(delay=delay)
        return next(filter(lambda x: x["user"]["id"] == str(user_id), self.members))

    def getCategories(self, *args, **kwargs):
        """Get all categories (TYPE = 4)"""
        if not self.categories:
            path = f'/guilds/{self.GUILD_ID}/channels'
            categories = list(filter(lambda c: c["type"] == 4, self.get(path)))
    ...
    def getChannels(self, *args, **kwargs):
        ...
        if not self.channels:
            path = f'/guilds/{self.GUILD_ID}/channels'
            self.channels = sorted(self.get(path), key=lambda x: x['name'].lower())
        return self.channels

    def getTextChannels(self, *args, **kwargs):
        """Get all Text Channels (type == 0)"""
        if not self.channels:
            self.getChannels
        return list(filter(lambda c: c["type"] == 0, self.channels))
    ...
    def getChannel(self, name='', category_id=0):
        """Find the channel(s), then add messages[] :) """
    ...
    def modifyChannel(self, channel_id=0, **kwargs):
        """Modify Channel Parameters"""
        payload = {**kwargs}
        path = f'/channels/{channel_id}'
        return self.patch(path, json=payload)
    ...
    def editMessage(self, channel_id=0, message_id=0, **kwargs):
    ...
    def deleteMessages(self, channel_id=0):
        """Delete all messages in a channel"""
        path = f'/channels/{channel_id}/messages/bulk-delete'
        return self.post(path=path)

    ###########################################################################
    #                         TO-DO: CLEAN UP users()                         #
    ###########################################################################
