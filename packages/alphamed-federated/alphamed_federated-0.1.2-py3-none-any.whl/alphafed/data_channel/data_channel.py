...
from .. import logger
from ..contractor import (AcceptSendingDataEvent, DenySendingDataEvent,
...
class DataChannel(ABC):
    ...
    @abstractmethod
    def receive_stream(self, session_id: str, source: str, sender_key: bytes) -> bytes:
        """Receive data stream from a sender.

        :args
            :session_id
                the session id of this data transmission
            :source
                the data sender
            :sender_key
...
class GRPCDataChannel(DataChannel):
    ...
    def receive_stream(self, session_id: str, source: str, sender_key: bytes) -> bytes:
        ...
        assert session_id, 'must specify the session_id'
        assert source, 'must specify the source'
        assert sender_key, 'must specify the public key of the sender'

        if not self._is_ready_for_receiving():
            self.contractor.deny_sending_data(session_id=session_id)

        # prepare and hand-shake
        port, private_key = self._prepare_for_connecting(session_id=session_id,
                                                         source=source,
        ...
        with TemporaryFile() as fp:
            ...
            data_channel_pb2_grpc.add_GRPCDataChannelServicer_to_server(
                servicer=GRPCDataChannelService(session_id=session_id,
                                                fp=fp,
