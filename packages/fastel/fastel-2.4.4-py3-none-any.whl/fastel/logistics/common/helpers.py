...
class SFLogisticsCreate:
    ...
    def parse_logistics(
        ...
        if order.logistics_type == LogisticTypes.OVERSEA.value:
            ...
            if order.logistics_subtype == OverseaSubtype.HK:
                receiver_country = OverseaSubtype.HK.value
                receiver_region_first = "香港"
            elif order.logistics_subtype == OverseaSubtype.MO:
                receiver_country = OverseaSubtype.MO.value
                receiver_region_first = "澳門"
            elif order.logistics_subtype == OverseaSubtype.CN:
                receiver_country = OverseaSubtype.CN.value
                receiver_region_first = "中國"
            elif order.logistics_subtype == OverseaSubtype.MY:
                receiver_country = OverseaSubtype.MY.value
                receiver_region_first = "馬來西亞"
            elif order.logistics_subtype == OverseaSubtype.SG:
                receiver_country = OverseaSubtype.SG.value
                receiver_region_first = "新加坡"
            elif order.logistics_subtype == OverseaSubtype.TH:
                receiver_country = OverseaSubtype.TH.value
                receiver_region_first = "泰國"
            else:
                raise APIException(
                    status_code=400, error="Invalid_Oversea_Subtype", detail=""
                )
        else:
            receiver_country = "TW"
...
class SFLogisticsStatus:
    ...
    def parse_gateway_resp(gateway_resp: Dict[str, Any]) -> LogisticsTrackStatusModel:
        resp_data = gateway_resp.get("data", {})
        if gateway_resp.get("success", False):
            track_details: List[Dict[str, Any]] = resp_data.get("tackDetailItems", [])
            logistics_id = resp_data.get("sfWaybillNo", "")
            logistics_message = ""
            if track_details:
                ...
                logistics_message = resp_data.get("trackSummary", "")
