...
try:
    from pdb_profiling.processors.uniprot.api import MapUniProtID, UniProtINFO
...
def downloadUnpSeq(ctx, input, idcol, sep, include):
    def processor(iterator: Iterator[Unfuture]):
        ...
        for task in iterator:
            yield task.then(UniProtINFO.process)

    click.echo(colorClick("UniProt Sequence Download"))
    UniProtINFO.logger = ctx.obj['logger']
    UniProtINFO.params['include'] = include
    if input:
        path = Path(input)
        ...
        UniProtINFO.retrieve(unps, ctx.obj['fasta_folder'], ctx.obj['unp_concurreq'],
                              ctx.obj['unp_concurrate'], semaphore=ctx.obj['semaphore'])
        return
    else:
        UniProtINFO.obj = {key: ctx.obj[key]
