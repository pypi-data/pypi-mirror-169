...
class CovarianceGenerator(object):
    ...
    @lru_cache(maxsize=10)
    def __init__(self, freqs, uv_bins):
...
class MRBF(MutliStationaryKern, MultiKern, RBF):

    def __init__(self, variance=1, lengthscale=1, ls_alpha=0, var_alpha=0, name='mrbf'):
        MutliStationaryKern.__init__(self, RBF, variance=variance, lengthscale=lengthscale,
                                     ls_alpha=ls_alpha, var_alpha=var_alpha, name=name)


class MMat32(MutliStationaryKern, MultiKern, Matern32):

    def __init__(self, variance=1, lengthscale=1, ls_alpha=0, var_alpha=0, name='mmat32'):
        MutliStationaryKern.__init__(self, Matern32, variance=variance, lengthscale=lengthscale,
                                     ls_alpha=ls_alpha, var_alpha=var_alpha, name=name)


class MMat52(MutliStationaryKern, MultiKern, Matern52):

    def __init__(self, variance=1, lengthscale=1, ls_alpha=0, var_alpha=0, name='mmat52'):
        MutliStationaryKern.__init__(self, Matern52, variance=variance, lengthscale=lengthscale,
                                     ls_alpha=ls_alpha, var_alpha=var_alpha, name=name)


class MExponential(MutliStationaryKern, MultiKern, Exponential):

    def __init__(self, variance=1, lengthscale=1, ls_alpha=0, var_alpha=0, name='mexp'):
        MutliStationaryKern.__init__(self, Exponential, variance=variance, lengthscale=lengthscale,
                                     ls_alpha=ls_alpha, var_alpha=var_alpha, name=name)
...
def get_prior_transform_ppf(prior):
    if isinstance(prior, Uniform):
        ...
        return lambda x: (10 ** prior.upper - 10 ** prior.lower) * x + 10 ** prior.lower
...
class SamplerResult(object):
    ...
    def plot_corner(self, fig=None, plot_prior=True, **kargs_corner):
        ...
        if plot_prior:
            ...
            self.pp = ParamaterPriors(self.gp.kern)
            prior_quantiles = self.pp.prior_transform(np.array([[0.025, 0.16, 0.84, 0.975],] * self.get_n_params()))

            for i, qs in enumerate(prior_quantiles):
                    ax = axes[i, i]
                    ax.axvline(qs[0], color="green", ls='-', lw=4, alpha=0.5)
                    ax.axvline(qs[1], color="green", ls='-', lw=4, alpha=0.25)
                    ax.axvline(qs[2], color="green", ls='-', lw=4, alpha=0.25)
                    ax.axvline(qs[3], color="green", ls='-', lw=4, alpha=0.5)
...
class MCMCSampler(AbstractSampler):
    ...
    def run(self, n_steps, verbose=False, live_update=False):
        ...
        if live_update:
            ...
            prior_quantiles = self.pp.prior_transform(np.array([[0.025, 0.16, 0.84, 0.975],] * self.ndim))
            ...
            for i, ax in zip(range(self.ndim + 1), axs.flatten()):
                ...
                if i < self.ndim:
                    ax.axhline(prior_quantiles[i, 0], c=psutil.dblack, ls='--', lw=1)
                    ax.axhline(prior_quantiles[i, -1], c=psutil.dblack, ls='--', lw=1)
                    ax.axhline(prior_quantiles[i, 1], c=psutil.lblack, ls='--', lw=1)
                    ax.axhline(prior_quantiles[i, -2], c=psutil.lblack, ls='--', lw=1)
            fig.tight_layout(pad=0.15)
