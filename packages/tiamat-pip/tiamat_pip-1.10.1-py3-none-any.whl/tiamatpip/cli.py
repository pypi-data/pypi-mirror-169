...
def process_pip_argv(argv: List[str]) -> None:
    ...
    with debug_print(
        ...
        with patched_environ(environ=environ):
            process_argv(argv)
    raise Exception("This function should never return")
...
def _run_code(source, filename=None, exec_locals=None):
    ...
    with prepend_sys_path(os.getcwd(), excludes=excludes):
        _handle_runpy_return(runpy._run_code, code, exec_locals)  # type: ignore[attr-defined]


def _handle_runpy_return(runpy_f, *args, **kwargs):
    try:
...
def run_python_file(argv: List[str]) -> None:
    ...
    with debug_print("run_python_file", argv):
        python_file = argv[0]
        with patched_sys_argv(argv):
            _handle_runpy_return(runpy.run_path, python_file, run_name="__main__")
