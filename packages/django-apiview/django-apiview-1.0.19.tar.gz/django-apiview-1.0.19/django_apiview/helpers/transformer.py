...
class body_alias(ApiviewDecorator):
    ...
    def process(self, _django_apiview_func, _django_apiview_request):
        _django_apiview_request.data[self.variable_name] = _django_apiview_request.data.get("_payload", _django_apiview_request.data.get("_form", None))
...
class cookie_variable(ApiviewDecorator):
    ...
    def process(self, _django_apiview_func, _django_apiview_request):
        value = _django_apiview_request.request.COOKIES.get(self.cookie_name, baseutils.Null)
        if value is baseutils.Null: # 如果cookie中不存在cookie_name所对应的值，则value需要设置为self.default
            value = self.default
        if value != baseutils.Null: # 如果self.default也没有有效值，则ApiviewRequest.data中不加有效的self.variable_name的键值对。注意：控制器有可能因为缺少必要参数而报错。
...
class meta_variable(ApiviewDecorator):
    ...
    def process(self, _django_apiview_func, _django_apiview_request):
        ...
        if value is baseutils.Null: # 如果meta中不存在meta_name所对应的值，则value需要设置为self.default
            value = self.default
        if value != baseutils.Null: # 如果self.default也没有有效值，则ApiviewRequest.data中不加有效的self.variable_name的健值对。注意：控制器有可能因为缺少必要参数而报错。
            _django_apiview_request.data[self.variable_name] = value
        return super().process(_django_apiview_func, _django_apiview_request)
