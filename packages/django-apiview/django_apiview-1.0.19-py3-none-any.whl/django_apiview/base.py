...
class ApiviewWrapper(object):
    ...
    def process(self, _django_apiview_func, _django_apiview_request):
        if hasattr(_django_apiview_func, "_django_apiview_wrapper") and getattr(_django_apiview_func, "_django_apiview_wrapper"):
            return _django_apiview_func(_django_apiview_request)
        else:
            logger.debug("{klass} calling funcutils.call_with_inject: func={func} data={data}".format(
                klass=self.__class__.__name__,
                func=_django_apiview_request,
                data=_django_apiview_request.data,
            ))
            result = funcutils.call_with_inject(_django_apiview_func, _django_apiview_request.data)
            logger.debug("{klass} call funcutils.call_with_inject done, result={result}".format(
                klass=self.__class__.__name__,
                result=result,
            ))
            return result
...
class Apiview(ApiviewWrapper):
    ...
    def __call__(self, _django_apiview_func):
        ...
        @cacheutils.cache(_django_apiview_func, "_django_apiview_final_func") # _django_apiview_func引入plugin包装后最终计算函数，只能缓存至_django_apiview_func中
        ...
        def wrapper(request, **kwargs):
            ...
            package = {}
            try:
                result = self.process(_get_django_apiview_final_func(), _django_apiview_request)
