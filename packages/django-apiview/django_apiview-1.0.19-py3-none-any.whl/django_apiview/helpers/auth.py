...
class check_aclkey(ApiviewDecorator):
    ...
    def __init__(self, aclkey=None, aclkeys=None, aclkey_field_name="aclkey"):
        ...
        self.aclkey = aclkey or getattr(settings, "DJANGO_APIVIEW_ACLKEY", None) # 设置中如果DJANGO_APIVIEW_ACLKEY为空，则禁用DJANGO_APIVIEW_ACLKEY的检验。检验禁用，表示检验不通过。
    ...
    def _check_aclkey(self, aclkey, _django_apiview_request):
        ...
        if callable(self.aclkey):
            result = funcutils.call_with_inject(self.aclkey, _django_apiview_request.data)
            if result:
                return True
        else:
            if self.aclkey and (self.aclkey == aclkey): # 如果self.aclkey为空，则针对self.aclkey的检验不通过。
                return True
