...
class Graph:
    ...
    def __hash__(self):
        if not self.hash:
            self.hash = int.from_bytes(sha256(str(self).encode()).digest())
        return self.hash
    ...
    def traverse(self, *args: Union[Node, BaseModel], outputs: List[str] = []) -> dict:
        ...
        self.trigger_event(TraversalEventEnum.TraversalStarted, state=state.value)
        ...
        while not complete:
            ...
            for path, value in flattened_state:
                ...
                for edge in edges:
                    ...
                    self.trigger_event(TraversalEventEnum.EdgeEnded, value=value)

        self.trigger_event(TraversalEventEnum.TraversalEnded)
