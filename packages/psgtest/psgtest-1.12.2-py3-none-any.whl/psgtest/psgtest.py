...
version = '1.12 Released 21-Aug-2022'
...
def settings_window():
    ...
    custom_interpreter_name = sg.user_settings_get_entry('-custom interpreter name-', '')
    custom_interpreter_path = sg.user_settings_get_entry('-custom interpreter path-', '')
    ...
    layout = [[sg.T('Program Settings', font='_ 25'), sg.Image(data=sg.EMOJI_BASE64_PONDER)],
              ...
              [sg.Frame('Python Interpreters (path to each python executible)', [[sg.Radio('', 1, k=(interpreter_dict[k],'-RADIO-')), sg.T(k, s=(5, 1)),
                                                                                ...
                                                                                [[sg.T('Custom interpreter:')],
                                                                                 [sg.R('', 1, k=('-P CUSTOM-', '-RADIO-')), sg.In(custom_interpreter_name, s=12, k='-P CUSTOM NAME-'), sg.In(custom_interpreter_path, k='-P CUSTOM PATH-'), sg.FileBrowse()]], font='_ 14')],
    ...
    interpreter_radio_key = (interpreter_dict[current_interpreter], '-RADIO-') if current_interpreter in interpreter_dict.keys() else ('-P CUSTOM-', '-RADIO-')
    ...
    while True:
        ...
        if event == 'Ok':
            ...
            sg.user_settings_set_entry('-custom interpreter name-', values['-P CUSTOM NAME-'])
            sg.user_settings_set_entry('-custom interpreter path-', values['-P CUSTOM PATH-'])
            ...
            if values[('-P CUSTOM-', '-RADIO-')] is True:
                current_interpreter = values['-P CUSTOM NAME-']
                current_interpreter_path = values['-P CUSTOM PATH-']
            else:
...
def make_window(sp_to_mline_dict=None, sp_to_filename=None):
    ...
    if sg.user_settings_get_entry('-custom interpreter name-', None):
            interpreter_list.append(sg.user_settings_get_entry('-custom interpreter name-'))
    ...
    while True:
        ...
        if isinstance(event, tuple):
            ...
            if values['-INTERPRETER TOP-'] in interpreter_dict.keys():
                ...
                interpreter_path = sg.user_settings_get_entry('-custom interpreter path-', '')
