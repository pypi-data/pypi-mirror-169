"""module description
    ...
    - feature db row id: 1 browser autostart, 2 blacklist, 3 html animation, 4 html style
    - feature_blacklist_switch_status() create (INSERT) or Update status feature enabled 1 or not 0
    ...
    - blacklist_writer thread collects new titles from (active) recorders and updates their radio blacklists
...
def blacklist_enabled_button_outfit_get(is_enabled):
    ...
    change btn color int Tools menu Monitor Records to reflect status
...
def update_radios_blacklists():
    ...
    get filename (title.mp3) title dict; if new write it to the radio db blacklist, update radios api blacklist dict
    ...
    have to check if title is the same as in last function call to update_radios_blacklists() (this function here)
    to not permanently open the db and compare if list contains title, is_new_in_title_compare_dict {radio name: title}
    not really an issue here, but fine in case of permanently polling cloud db
    """
    ...
    for radio, new_title in recorder_new_title_dict_cp.items():
        if radio in is_new_in_title_compare_dict.keys():
            ...
            if recorder_new_title_dict_cp[radio] == is_new_in_title_compare_dict[radio]:
                continue    # no db access necessary, title has not changed after last compare
