...
def index():
    ...
    POST: js send post with button click event listener caught pressed button, / index endpoint return json dict
    js
    activate, reset, reload audio element src /deactivate audio
    sound route endpoint to connect audio element
    id for js auto click abandoned button
    radio name to draw on html console

    GET: jinja vars for non POST call
    posts - db dump
...
def degrade_animation_level_set():
    ...
    config_html.tools_feature_settings_get_rows is called on startup to write all rows if version upgrade
...
def display_info():
    """returns a dict with radio {id: message},
