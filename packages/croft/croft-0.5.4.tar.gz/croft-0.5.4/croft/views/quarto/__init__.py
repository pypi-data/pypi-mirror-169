...
<% for (const tile of items) { %>
  <div class="card border-2 rounded-3 g-col-12 g-col-sm-6 g-col-md-4 mb-2" <%= m
etadataAttrs(tile) %>>
   <div class="card-body">
    ...
    <% if (tile.image) { %>
      <a href="<%- tile.path %>">
        <img src="<%- tile.thumbnail %>" alt="<%- tile.description %>" class="ca
rd-img-top"/>
    </a>
    <% } else { %>
...
"""
            ...
            with open(f"{dir_base}/content/mysite/map/{public_name}/gallery.ejs", "w") as resource_file:
...
#                  hugo_file.write(f"year: \"{year_field}\"\n")
              ...
              hugo_file.write("---\n")
              hugo_file.write("I think this then appears as the content\n")
...
BB3hKbKWOc9Ez/TAwyWxNXeoV9c1v6FIeYiBieIWkpLjauysF18NzgR1MBNBXf8/KABdlkX68nAhlwcD
FLGPCQ==" crossorigin=""></script>
...
<script>
	...
	var tiles = L.tileLayer('https://api.mapbox.com/styles/v1/{id}/tiles/{z}
/{x}/{y}?access_token=pk.eyJ1IjoibWFwYm94IiwiYSI6ImNpejY4NXVycTA2emYycXBndHRqcmZ
3N3gifQ.rJcFIG214AriISLbB6B5aw', {
		maxZoom: 18,
