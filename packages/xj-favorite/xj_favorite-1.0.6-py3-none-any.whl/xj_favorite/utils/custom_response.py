...
# 数据返回规则
def util_response(data='', err=0, http_status=status.HTTP_200_OK, request='', msg='ok'):
    if http_status == status.HTTP_200_OK:
        return Response({'err': err, 'msg': msg, 'data': data, "request": request})
    else:
        return Response({'err': http_status, 'msg': msg, 'request': request}, status=http_status)
