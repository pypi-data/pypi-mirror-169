...
class System:
    ...
    def __init__(self, config, name, principal, service_account, conn_string, proxy_rewrite_rule, containers, identifier, services={}, security_context={}, enable_init_container="false"):
        ...
        print(f"ENABLE INIT CONTAINER =================> {self.enable_volume_permissions_init_container}")
    ...
    def enable_init_container(security_context):
        ...
        if os.environ.get("ENABLE_VOLUME_PERMISSIONS_INIT_CONTAINER", "false").lower() == "true":
            if all(required_key in security_context for required_key in required_keys):
                for key in required_keys:
                    if security_context[key]:
                        continue
                    else:
                        return "false"
                return "true"
    ...
    def parse (config, name, principal, system, service_account, env={}, services={}):
        ...
        """
        system["security_context"] = {"runAsGroup": 8931}
