...
from gb_lib.GbLib import GB_LOG_HOME_ENV_KEY
from log_ocr import GbOcrTrack
...
class AORunActivityLog:
    ...
    _activity_db_logger: GbOcrTrack
    ...
    def __init__(self, prefix: str, level: int, home: str, run_descriptor: str = "runtime",
        ...
        if home is None:
            _ = os.getenv(GB_LOG_HOME_ENV_KEY)
            home = _ if _ is not None else os.getcwd()
    ...
    def reset(self):
        ...
        activity_formatter = logging.Formatter(fmt='%(asctime)s:%(message)s', datefmt='%m-%d-%YT%H-%M-%S')
        ...
        self.activity_db_logger = GbOcrTracker()
    ...
    def activity_descriptor(self):
        return self._activity_descriptor

    @activity_descriptor.setter
    def activity_descriptor(self, value):
        self._activity_descriptor = value
        self.reset()
