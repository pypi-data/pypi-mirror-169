...
class PokeClient:
    ...
    def get_evolution_chain(self, id: int) -> Dict[str, Any]:
        """
        ...
        :type id: int
        :raises PokeError: Evolution chain not found
        :return: Evolution chain information
        :rtype: Dict[str, Any]
        """
        url = f"{self._base_url}evolution-chain/{id}"
        response = requests.get(url)
        if response.status_code == 200:
            return response.json()
        else:
            raise PokeError(response.status_code, "Evolution chain not found")

    def get_pokemon_move(self, id: int) -> Dict[str, Any]:
        """
        ...
        :param id: Move ID
        :type id: int
        :raises PokeError: Move not found
        :return: Move information
        :rtype: Dict[str, Any]
        """
        url = f"{self._base_url}move/{id}"
        response = requests.get(url)
        if response.status_code == 200:
            return response.json()
        else:
            raise PokeError(response.status_code, "Move not found")