...
class ptf_analysis():
    ...
    def create_breakpoint(self, data, number, perc=None):
        '''
        input: 
            data(ndarray/DataFrame) : data having structure
                first row to last two row: character
                last row : time index
            number (int) : the number of breakpoints
            perc (array) : the percentiles of breakpoints
        output:
            breakpoints of characters
        '''
        import numpy as np
...
#            return self.average, self.ttest, self.alpha, self.alpha_tvalue
    ...
    def extractor(self, pos):
        '''
        This function extract the return series
        input:
        ...
        for i in range(len(groups_time)):
            ...
            if self.perc_sign == False:
                if self.weight == False:
                    ...
                    try:
                        average_group_time[:,:,i] = super().average(group[:, 0], label, cond='bi', weight=group[:, -1]).reshape((self.num_row, self.num_col))
                    except:
                        average_group_time[:,:,i] = np.full([self.num_row, self.num_col], np.nan)
    ...
    def print_summary(self, explicit=False, export=False, percentage=False):
        ...
        if self._factor is None :
            ...
            if self._sample_type == 'ndarray':
                table.field_names = ['Group'] + [i+1 for i in range(self.num_row)] + ['Diff']
            elif self._sample_type == 'DataFrame':
                table.field_names = ['Group'] + [self._columns[2] + str(i+1) for i in range(self.num_row)] + ['Diff']
