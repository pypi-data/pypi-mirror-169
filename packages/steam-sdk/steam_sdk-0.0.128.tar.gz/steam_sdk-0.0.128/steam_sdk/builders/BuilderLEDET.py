...
class BuilderLEDET:
    ...
    def __init__(self, path_magnet: Path = None, input_model_data=None,
        ...
        if self.case_model == 'magnet':
            ...
            if flag_build:
                ...
                if type_windings in ['multipole', 'solenoid', 'busbar']:  # notably, CCT_straight geometry is not handled here
                ...
                if self.model_data.GeneralParameters.magnet_type == 'CCT_straight':
                    ...
                    group_to_coil_section = self.model_data.CoilWindings.group_to_coil_section
                    polarities_in_group = self.model_data.CoilWindings.polarities_in_group
                    n_half_turn_in_group = self.model_data.CoilWindings.n_half_turn_in_group
                    half_turn_length = self.model_data.CoilWindings.half_turn_length
                    # if all(entry is None for entry in [conductor_to_group, group_to_coil_section, polarities_in_group, n_half_turn_in_group, half_turn_length]):
                    if len(conductor_to_group) == 0:
                        if self.model_data.Quench_Protection.FQPLs.enabled:
                            fqpls_iterator = self.model_data.Quench_Protection.FQPLs.currents
                        else:
                            fqpls_iterator = []
                        cct_straight = self.model_data.CoilWindings.CCT_straight
                        iterator = cct_straight.winding_numberTurnsFormers + fqpls_iterator
                        nhtig = [t * r * c for t, r, c in zip(cct_straight.winding_numberTurnsFormers, cct_straight.winding_numRowStrands, cct_straight.winding_numColumnStrands)] + [1 for _ in fqpls_iterator]
                        list_of_ones = [1 for _ in iterator]
                        list_of_zeros = [0.0 for _ in iterator]
                        self.model_data.CoilWindings.conductor_to_group = list_of_ones
                        self.model_data.CoilWindings.group_to_coil_section = list_of_ones
                        self.model_data.CoilWindings.polarities_in_group = list_of_ones
                        self.model_data.CoilWindings.n_half_turn_in_group = nhtig
                        self.model_data.CoilWindings.half_turn_length = list_of_zeros
                    else:
                        pass
    ...
    def calcElectricalOrder(self, flag_plot: bool = False):
        """
            **Calculates electrical order of each half turn (for multipole magnets) or of each turn (for solenoid and CCT_straight magnets) **
        ...
        if typeWindings in ['multipole', 'busbar']:
            ...
            fqpl_names = self.model_data.Quench_Protection.FQPLs.names
