    ...
    def __init__(self, path_magnet: Path = None, input_model_data=None,
        ...
        if self.case_model == 'magnet':
            ...
            if flag_build:
                ...
                if type_windings in ['multipole', 'solenoid', 'busbar', 'CCT_straight']:  # notably, CCT geometry is not handled here
    ...
    def setThermalConnections(self):
        ...
        if typeWindings in ['multipole', 'solenoid', 'busbar']:
            ...
            if len(fqpl_names)>0:   # only if any fqpls have been defined and/or are enabled
                ...
                # check if teh above function produced fqpl_th_conns_def that is of the right shape
                for fqpl_i in range(len(fqpl_th_conns_def_bool)):  # for each fqpl
                ...
                th_conns_width_fqpl = []        # no fqpls are defined or enabled so an empty array for looping below
