        ...
        **kwargs           - any parameters of self.keras_predict()
        ...
        for i in range(self.FORECAST_LENGTH):
            ...
            with self.HiddenPrints():
                ...
                if predict_method == 'Single': df_result = self.single_keras_predict(data=self.TARGET, save_result=save_each_result, **kwargs)
                if predict_method == 'Ensemble': df_result = self.ensemble_keras_predict(data=self.TARGET, save_result=save_each_result, **kwargs) 
                if predict_method == 'Respective': df_result = self.respective_keras_predict(data=self.TARGET, save_result=save_each_result, **kwargs)
                if predict_method == 'Hybrid': df_result = self.hybrid_keras_predict(data=self.TARGET, save_result=save_each_result, **kwargs)
            time2 = time.time()
            print('taking time: %.3fs'%(time2-time1))
            df_result['Run'] = i
