...
class ChannelProvider:
    ...
    def get_stix_channel(self):
        if self._stix_channel is None:
            ...
            new_channel.queue_declare(queue='stix-queue', exclusive=True)            

            new_channel.queue_bind(exchange='stix', queue='stix-queue')

            self._stix_channel = new_channel
...
class Helper:
    ...
    def send_stix_bundle(self, bundle: Bundle) -> None:
        ...
        channel.basic_publish(
            exchange="stix", routing_key="stix-queue", body=bundle.serialize()
        )
        channel.close()
