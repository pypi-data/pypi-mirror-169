...
def POST_BULK(connection, schema, tableName:str, data:list, byCopy = True, onConflict:bool=False, uniqueColumnNamesForConflict:str='id'):
    ...
    if byCopy and not onConflict:
        ...
        result = herokuDbAccess.insertBulkDataIntoDatabase(queryString,template,newData,connection)
    return result
...
def PUT_BULK(connection, schema, updates:list, tableName:str, conditionColumns:list=['id']):
    ...
    for col in columns:
        if col not in conditionColumns:
            setArray.append("SET t.{} = tmp.{}".format(col,col))
    
    tmpTableName = 'tmp_' + tableName
    queryString = "UPDATE {{}}.{} t {} FROM {{}}.{} tmp WHERE 1=1 AND {}".format(tableName,', '.join(setArray),tmpTableName,' AND '.join(conditionArray))

    query = sql.SQL(queryString).format(sql.Identifier(schema),sql.Identifier(schema))
...
if __name__ == '__main__':
    ...
    for i in range(1,10):
        ...
        data.append(item)
    r = POST_BULK(connection,'dbdev','testbulk',data)
    print(r)
    ...
    for i in range(1,10):
        ...
        r= POST(connection,'dbdev','testbulk',data)
        print(r)
