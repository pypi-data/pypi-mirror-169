...
class ControlPanel(tk.Frame):
    ...
    def __set_widget_position(self, widget:tk.Widget, column:int=0):
        if column not in self.rows:
            self.rows[column] = 0
        widget.grid(column=column, row=self.rows[column], sticky=tk.NSEW)
        self.rows[column] += 1
    ...
    def create_menu(self, name:str, choices:List[Any], callback:Optional[Callable[[Any],None]]=None, column:int=0):
    ...
    def create_slider(self, name:str, start:float=0.0, end:float=1.0, step:float=0.01, default:float=None, show_value:bool=True, callback:Optional[Callable[[Any],None]]=None, column:int=0):
