...
class loops:
    ...
    def CHANNEL_LISTENER(self, channel, function, refresh=1, type=0):
        if type == 0:
            Thread(target=lambda: self.CHANNEL_LISTENER(channel, function)).start()
            return hash(channel+str(function))
        else:
            scanner = autocord.utils(self.autocord)
            last_scanned = scanner.FETCH_MESSAGE_HISTORY(channel, limit=1)[0]

            # add to listeners
            self.listeners[str(hash(channel+str(function)))] = {
                'channel': channel,
                'function': function
            }
            ...
            while end is False:
                ...
                if channel_messages[0].id != last_scanned.id:
                    ...
                    # set last scanned
                    last_scanned = channel_messages[i]

                # refresh to check loop again
                for i in range(refresh):
                    if not str(hash(channel+str(function))) in self.listeners:
                        end = True
                        break
                    time.sleep(1)
