...
from textractor.utils.search_utils import get_word_similarity, jaccard_similarity
...
class Document(SpatialObject):
    ...
    # functions to add entities to the Document object
    @property
    ...
    def search_word(
        ...
        for word in self.words:
            similarity = get_word_similarity(keyword, word.text, similarity_metric)
            similarity = (
                similarity
                if similarity_metric == SimilarityMetric.COSINE
                else -(similarity)
    ...
    def search_line(
        ...
        for line in self.lines:
            similarity = [
                get_word_similarity(keyword, word, similarity_metric)
                for word in line.__repr__().split(" ")
            ]
            similarity.append(
                get_word_similarity(keyword, line.__repr__(), similarity_metric)
            )
            similarity = (
                max(similarity)
                if similarity_metric == SimilarityMetric.COSINE
    ...
    def get_value_by_key(
        ...
        for kv in self.key_values + self.checkboxes:
            ...
            similarity = [
                get_word_similarity(key, word, similarity_metric)
                for word in edited_document_key.split(" ")
            ]
            similarity.append(
                get_word_similarity(key, edited_document_key, similarity_metric)
            )
    ...
    def export_tables_to_excel(self, filepath):
        ...
        for table in self.tables:
            workbook = table.export_as_excel(
