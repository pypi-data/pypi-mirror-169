...
def pathways_to_genes(rm_paths, species, all_features):
    ...
    new_remove_features = list(set(new_remove_features))
    return(new_remove_features)


    # ########################################################################
    # remove_dict=process_dict(results,3)
    # #############################################################
    # all_rm_genes = list(remove_dict.keys())
    # print('\tfound',len(all_rm_genes),'genes to remove')
...
def get_all_remove_genes(pre_remove_features,
    ...
    if min_express_n==None:
        ...
        min_express_n=min(50,int(0.05*exprs.shape[cell_axis]))
    ...
    for gene in all_features:
        if gene in pathway_removed_genes:
...
def get_the_spears(exprs_subset_file, spear_file, kept_features, bin_size, n_rand_feat, FPR, FDR, annotation_df, num_pos_cor=10):
    ...
    #print(feature_table[feature_table["selected"]==True])
    ## TODO - left off here
...
def get_anti_cor_genes(exprs,
    ...
    spear_f.close()
    ##########
    feature_table = get_the_spears(exprs_subset_file, spear_file, kept_features, bin_size, n_rand_feat, FPR, FDR, annotation_df, num_pos_cor=num_pos_cor)
    ##########
...
# def get_anti_cor_genes_from_tsv(infile):
