...
def fit(excel_name,file_result,f_fit,popt_names_latex,variabile_x,variabile_y,sigma_y,
    ...
    y_chi=0,
    post_decimal_chi=0,
    post_decimal_params=None,
    log_to_file=False,
    debug_mode=False,#disattiva latex e manda i risultati in console
    path=".",
    xscale="linear",
    yscale="linear",
    lim_inf=None,
    ...
    pad_percent=0.10,
    tratteggio=True,
    spessore_zero_line=1,
    precisione_curvatura=1001,
    foglio="Foglio1",
    funzioni_modifica=[None, None],
    residui_ratio=[4.236067977500615, 1],#golden ratio
    fig_dimension=(6.24136,4.4), #pdf page latex dimension
    ...
    popt_names_latex : list
        ...
        devono essere scritti in Latex, la stringa deve essere composta da una parentesi graffa contenete il nome del parametro seguito
        da una quadra contenente l'unità di misure [\si ...]
    ...
    post_decimal_params: list of int, optional
        numero di cifre dopo la virgola con cui scrivere i parametri di fit nella tabella, se None allora tutte le cifre vengono esplicitate (default: None)
    ...
    debug_mode : bool, optional
        if True il grafico è disegnato senza compilare in latex e le info sono scritte in console (default: False)
    ...
    tratteggio : bool, optional
        se True indica che i valori aggiunti da "pad_percent" vengano rappresentati prolungando la linea del grafico con un tratteggio (default: True)
    ...
    popt_sigma=np.sqrt(pcov.diagonal())
    ...
    if debug_mode:
        print("debug mode: ON")
        print("parametri ottimali di fit: ")
        print(popt)
        print("errori associati: ")
        print(popt_sigma)
    ...
    for element_y, element_t, element_dy in zip(y, t, sigma_eff):
        ...
        if np.absolute(element_y-f_fit(element_t,*popt))>element_dy:
            lontano_fit+=1
    p_value=stats.distributions.chi2.sf(chi_rid, gradi_di_liberta)

    frase_log=["","","analisi del fit:",""]
    ...
    frase_log.append("")
    frase_log.append("Numero di punti che distano dal grafico n>1 barre di errore: "+str(lontano_fit)+" (expected-nell'ipotesi di errori gaussiani-: "+str(0.32 *len(t))+")")
    frase_log.append("")
    frase_log.append("")
    frase_log.append("Il chi quadro vale: "+str(chisq))
    frase_log.append("per un numero di gdl/dof di: "+str(gradi_di_liberta))
    frase_log.append("")
    if p_value<0.5:
        frase_log.append("probability of obtaining a more extreme value is: "+str(p_value))
    else:
        frase_log.append("probability of obtaining a more extreme value is: "+str(1-p_value))
    frase_log.append("probability of 22 heads on a coin flip is: 2.38e-07 (2.87 valore gaussiano per >5 sigma)")

    if log_to_file and not debug_mode:
    ...
    if not debug_mode:
        colonne="c"*len(popt)*2 # colonne centrate in numero pari al numero di parametri
        line_begin=r"\begin{longtable}{"+colonne+r"}"

        popt_names_latex=[r"\(\hat"+str(e)+r"\) & \(\sigma"+str(e)+"\)" for e in popt_names_latex]
        table_header="&".join(popt_names_latex)+r"\\"
        longtable_header=[r"\toprule",table_header ,r"\midrule", r"\endfirsthead", r"\toprule",table_header ,r"\midrule",r"\endhead", r"\midrule", r"\multicolumn{"+str(len(popt)*2)+r"}{r}{{continua a pagina seguente}} \\", r"\midrule", r"\endfoot", r"\bottomrule", r"\endlastfoot"]
        if post_decimal_params:
            string_popt=[str(round(i_opt, post_decimal_params_i)) +"&" + str(round(sigma_i_opt, post_decimal_params_i)) for i_opt,sigma_i_opt, post_decimal_params_i in zip(popt,popt_sigma,post_decimal_params)] #numeri convertiti in stringa
        else:
            string_popt=[str(i_opt) +"&" + str(sigma_i_opt) for i_opt,sigma_i_opt in zip(popt,popt_sigma)] #numeri convertiti in stringa
    ...
    ax2.plot(a1, a1-a1, color=res_line_color, linewidth=spessore_zero_line)
    ax2.plot(a2, a2-a2, color=res_line_color, linewidth=spessore_zero_line)
    ax2.plot(a3, a3-a3, color=res_line_color, linewidth=spessore_zero_line)
    ...
    if chi:
        if not debug_mode:
            if ridotto:
                ...
                ax1.text(x_chi,y_chi,r"$$\chi^2/\nu="+str(round(chisq,post_decimal_chi))+r"/"+str(gradi_di_liberta)+r"$$")
        else:
            ax1.text(x_chi,y_chi,"il chi quadro è qui")
