...
def test_json():
    ...
    os.makedirs(f"{local_output_dir}", exist_ok=True)
    tar = f"{BASE_PATH}/resources/carisSample.tar.gz"
    result = process_caris_json(tar, local_output_dir, INGEST_STATUS, mock_log)
    ...
    for extension in extensions:
        os.remove(f"{BASE_PATH}/resources/carisSample.{extension}")

    assert "somatic_vcf" or "germline_vcf" in result
