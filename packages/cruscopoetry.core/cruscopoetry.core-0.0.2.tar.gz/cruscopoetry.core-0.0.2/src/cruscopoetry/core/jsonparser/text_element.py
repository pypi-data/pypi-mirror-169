...
class JsonElement:
	...
	def iter_stanzas_dict(self):
		for array in self.jdict.iter("stanzas"):
			yield from array
	...
	def iter_lines_dict(self):
		for array in self.jdict.iter("lines"):
			yield from array
	...
	def iter_cola_dict(self):
		for array in self.jdict.iter("cola"):
			yield from array
	...
	def _words_iterator_split_on_boundaries(self):
		for array in self.jdict.iter("words"):
			yield from array
	...
	def iter_syllables_dict(self):
		for array in self.jdict.iter("syllables"):
			yield from array
	...
	def iter_phonemes_dict(self):
		for array in self.jdict.iter("phonemes"):
			yield from array
	...
	def iter_stanzas(self):
		for index, stanza in enumerate(self.iter_stanzas_dict()):
			yield self.__class__.STANZA_CLASS(index, stanza)
	
	def iter_lines(self):
		for index, stanza in enumerate(self.iter_lines_dict()):
			yield self.__class__.LINE_CLASS(index, stanza)
	
	def iter_cola(self):
		for index, stanza in enumerate(self.iter_cola_dict()):
			yield self.__class__.COLON_CLASS(index, stanza)
	
	def iter_words(self):
		for index, stanza in enumerate(self.iter_words_dict()):
			yield self.__class__.WORD_CLASS(index, stanza)
	
	def iter_syllables(self):
		for index, stanza in enumerate(self.iter_syllables_dict()):
			yield self.__class__.SYLLABLE_CLASS(index, stanza)
...
#############################
	...
	def __repr__(self):
		return "%s (%r)"%(self.__class__.__name__, self.jdict.keys())
...
class JsonNumberedElement(JsonElement):
	...
	def __repr__(self):
		return "%s-%d(%r)"%(self.__class__.__name__, self.number, list(self.jdict.keys()))
