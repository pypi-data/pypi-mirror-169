...
from ..abstractrenderer import AbstractNoTranslationDisposer, AbstractAfterTextDisposer, AbstractEachStanzaDisposer, AbstractEachLineDisposer
...
class PlainTextNoTranslationDisposer(AbstractNoTranslationDisposer, PlainTextDisposer):

	def __init__(self, line_renderers: Tuple[Tuple[PlainTextLineRenderer, ...]]):
		super().__init__(line_renderers)
		self.number_column_length = self._get_number_column_length(self.total_lines)

	def render_stanza(self, stanza: Tuple[PlainTextLineRenderer], number_column_length: int, line_numbers_after: int, indent: str) -> str:
		"""Gets the renderings of the lines and groups them in a multiline string."""
		return super().render_stanza_source(stanza, number_column_length, line_numbers_after, indent)		
...
class PlainTextAfterTextDisposer(AbstractNoTranslationDisposer, PlainTextDisposer):
	...
	def get_rendered_lines(self, line_numbers_after: int, indent: str):
		...
		source = self.render_text_source(self.line_renderers, self.number_column_length, line_numbers_after, indent)
		translation = self.render_text_translation(self.line_renderers, self.number_column_length, line_numbers_after, indent)

		ret_str = source + Joiners.SOURCE_AND_TRANSLATION*3 + translation
		return ret_str
		

class PlainTextEachStanzaDisposer(AbstractNoTranslationDisposer, PlainTextDisposer):
	...
	def get_rendered_stanza(self, stanza: Tuple[PlainTextLineRenderer, ...], line_numbers_after: int, indent: str):
		...
		source = self.render_stanza_source(stanza, self.number_column_length, line_numbers_after, indent)
		translation = self.render_stanza_translation(stanza, self.number_column_length, line_numbers_after, indent)
...
class PlainTextEachLineDisposer(AbstractNoTranslationDisposer, PlainTextDisposer):
	...
	def get_rendered_line(self, line: PlainTextLineRenderer, line_number_after: int, indent: str):
		source = self.render_line_source(line, self.number_column_length, line_number_after, indent)
		...
		translation = self.render_line_translation(line, self.number_column_length, None, indent)
