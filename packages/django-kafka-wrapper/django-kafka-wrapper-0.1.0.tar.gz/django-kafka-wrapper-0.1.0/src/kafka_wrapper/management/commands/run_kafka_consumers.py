...
from apps.kafka_wrapper.consumers import KeepRefsMixin
from apps.kafka_wrapper.settings import kafka_settings
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        self._yellow(_LOGO)
        self._yellow(f'API version: {".".join(map(str, kafka_settings.version))}')
        ...
        for consumer in itertools.cycle(self.consumers):
            try:
                try:
                    ...
                    self._yellow('Consumer error: ', ending='')
                    self._red(e)
                else:
                    try:
                        self._yellow('Starting job...')
