...
class RasterBlockSource(filebasedsource.FileBasedSource):
    ...
    array. Returns (`value`, `geom`) tuple for each block. `value` is a 3D
    ...
    def read_records(self, file_name, range_tracker):
        ...
        with rasterio.open(file_name) as src:
            ...
            while range_tracker.try_claim(next_pos):
                ...
                geom = transform_geom(src_crs, 'epsg:4326', {
                    'type': 'Polygon',
    ...
    def __init__(self, file_pattern, bidx=1, in_epsg=None, in_proj=None, **kwargs):
        ...
        super(RasterBlockSource, self).__init__(file_pattern, min_bundle_size=int(1e9))
...
class RasterPolygonSource(filebasedsource.FileBasedSource):
    ...
    is most suitable for ETL processes.
    """
    ...
    def read_records(self, file_name, range_tracker):
        ...
        with rasterio.open(file_name) as src:
            ...
            while range_tracker.try_claim(next_pos):
                ...
                win = block_windows[i]
                block = src.read(self.band_number, window=win)
                block_mask = src.read_masks(self.band_number, window=win)
    ...
    def __init__(self, file_pattern, bidx=1, in_epsg=None, in_proj=None, band_number=1,
            **kwargs):
        self.in_epsg = in_epsg
        self.in_proj = in_proj
        self.band_number = band_number

        super(RasterPolygonSource, self).__init__(file_pattern, min_bundle_size=int(1e9))
