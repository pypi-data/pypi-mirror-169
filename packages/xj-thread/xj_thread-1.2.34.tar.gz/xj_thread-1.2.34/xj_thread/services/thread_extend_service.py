...
class ThreadExtendOutPutService():
    ...
    def __init__(self, category_id_list=None, thread_id_list=None):
        ...
        for item in self.filed_list:
            if self.filed_map.get(item["category_id"]):
                self.filed_map[item["category_id"]].update({item["field_index"]: item["field"]})
            else:
                self.filed_map[item["category_id"]] = {item["field_index"]: item["field"]}
