...
import ..utils.rdi_adcp_utils as rdi_adcp_utils
import pdb
import importlib
...
class RDI_ADCP_PD02(xrwrap):
    ...
    def read_pd0_dd(self, folder, file_, nens=None, attributes={}):
        """
        Read PD0 file with either Dolfyn of Dall's porpoise. 
        """
        
        # Load the raw binary data
        data = rdi.read_rdi(self.fullpath) # This would be Levi Kilcher's dat object
        self._data = data

        if not self._data.config.coord_sys == 'beam':
    ...
    def _calc_rotations(self):
        ...
        # dimensions
        adcp_dims = {
            'distance':self._data.range.squeeze(),
            'time':self.time,
            'beam':list(range(1,5)),
        }
