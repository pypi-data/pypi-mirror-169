...
def confint(boot_set):
    ...
    for spd in boot_set:
        y = np.array(
            [[val[0], val[1]] for val in spd if (val[0] >= mini) and (val[0] <= maxi)]
        )
        yy = np.lib.pad(
            y.T[1],
            (np.int(y.T[0][0] - mini), np.int(maxi - y.T[0][-1])),
            "constant",
            constant_values=0,
        )
        yrange.append(yy)
...
class CalibrationCurve(np.ndarray):
    ...
    def __new__(cls, curve):
        try:
            ...
            try:
                with open(curve, encoding="latin-1") as curve_file:
                    title = curve_file.readline().strip("#\n")
            except FileNotFoundError:
                try:
                    curve_resource_filename = pkg_resources.resource_filename(
                        "iosacal", f"data/{curve}.14c"
                    )
                    with open(
                        curve_resource_filename, encoding="latin-1"
                    ) as curve_file:
                        title = curve_file.readline().strip("#\n")
                except FileNotFoundError:
                    available = pkg_resources.resource_listdir("iosacal", "data")
                    message = "\n".join(
                        [res[:-4] for res in available if res.endswith(".14c")]
                    )
            ...
            ud_curve_interp = np.array(
                [curve_arange, values_interp, stderr_interp]
            ).transpose()
    ...
    def mixing(self, curve2_name, P, D=0, deltaR=0, err_deltaR=0):
        ...
        self.T[0] = curve1.T[0]
        self.T[1] = (1.0 - P) * curve1.T[1] + P * curve2.T[1]
        self.T[2] = np.sqrt(
            np.power((1.0 - P) * curve1.T[2], 2)
            + np.power(P * curve2.T[2], 2)
            + np.power(D * (curve1.T[1] - curve2.T[1]), 2)
        )
...
class RadiocarbonDetermination(object):
    ...
    def calibrate(self, curve, norm=True, cutoff=5):
        ...
        curve = CalibrationCurve(curve)

        _calibrated_list = []

        # We apply the cutoff for the gaussian range in 14C scale
        idx = np.where(
            (curve.T[1] > self.date - cutoff * self.sigma)
            & (curve.T[1] < self.date + cutoff * self.sigma)
        )
        idxmin = np.min(idx)
        idxmax = np.max(idx)
        ...
        if norm == True:
            ...
            y = np.array(_calibrated_list).T[1] / np.sum(
                np.array(_calibrated_list).T[1]
            )
    ...
    def __str__(self):
        return "RadiocarbonSample( {id} : {date:.0f} ± {sigma:.0f} )".format(
            **self.__dict__
        )
...
class CalAge(np.ndarray):
    ...
    def __new__(cls, input_array, radiocarbon_sample, calibration_curve):
        ...
        obj.intervals = {68: hpd_interval(obj, 0.318), 95: hpd_interval(obj, 0.046)}
    ...
    def quantiles(self):
        """Return the median, one, and two sigma ages from a RadiocarbonDetermination object."""
        quantiles = {}

        # get cumulative sum of rows
        cumsum = self[:, 1].cumsum()

        # median is the row whose cumsum is closest to 0.5
        quantiles[50] = self[np.argmin(abs(cumsum - 0.5)), 0]

        # Add 1 & 2 sigma +/- to quantiles dictionary
        for q in [68, 95]:
            quantiles[q] = [
                self.intervals[q][-1].to_year,
                self.intervals[q][0].from_year,
            ]

        return quantiles

    def __str__(self):
        return 'CalAge( {radiocarbon_sample.id} based on "{calibration_curve.title}" )'.format(
            **self.__dict__
        )
...
def combine(determinations):
    ...
    # pooled mean
    pool_m = sum(mi / si**2 for mi, si in zip(m, s)) / sum(1 / si**2 for si in s)
...
def simulate(calendar_age, sigma, id, curve, BP="bp"):
    """Simulate a radiocarbon determination for a sample of a known date.

    This function simulates a radiocarbon determination from a lab for
    a sample of a known calendar age with an specified lab error.

    Parameters:
    calendar_age (int): Sample calendar age to be simulated.
    sigma (int): Expected measurement error from radiocarbon lab.
    id (str): ID of the sample.
    curve (str): Calibration curve as file path or short name e.g. "intcal20".
    BP (str): Specifies Gregorian (AD/CE) or BP date for `calendar_age` input.

    Returns:
    RadiocarbonDetermination: Simulated radiocarbon determination as reported by a lab.

    """

    if BP.lower() not in ["ce", "ad", "bp"]:
        raise ValueError(
            f"Specified date must be one of (CE, AD, BP). You provided {BP}."
        )

    curve = CalibrationCurve(curve)

    # Convert to BP if applicable.
    if BP.lower() in ["ce", "ad"]:
        calendar_age = 1950 - calendar_age

    # Get matching mean 14C age and uncertainty from calibration curve.
    idx = np.argwhere(curve[:, 0] == calendar_age).item()
    uncalibrated_mean, uncalibrated_std = curve[idx, 1:]

    # calculate combined std.
    combined_std = sqrt(uncalibrated_std**2 + sigma**2)

    date = stats.norm.rvs(loc=uncalibrated_mean, scale=combined_std).astype(int)

    return R(date, sigma, id)
