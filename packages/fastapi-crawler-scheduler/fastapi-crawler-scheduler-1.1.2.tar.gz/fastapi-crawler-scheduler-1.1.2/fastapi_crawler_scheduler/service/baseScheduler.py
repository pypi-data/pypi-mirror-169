...
from apscheduler.util import ref_to_obj, undefined, datetime_to_utc_timestamp
...
class BaseScheduler(object):
    ...
    def scheduler_add_job(
        ...
        job_kwargs = {
            'trigger': self.scheduler._create_trigger(trigger, dict(seconds=interval)),
            'executor': 'default',
            'func': func,
            'args': (),
            'kwargs': crawler_info,
        ...
        if self.redis_job_store.redis.hexists(self.redis_job_store.jobs_key, job.id):
            self.redis_job_store.remove_job(job_id=job_id)

        with self.redis_job_store.redis.pipeline() as pipe:
            pipe.multi()
            pipe.hset(self.redis_job_store.jobs_key, job.id, pickle.dumps(job.__getstate__(),
                                                                          self.redis_job_store.pickle_protocol))
            if job.next_run_time:
                pipe.zadd(self.redis_job_store.run_times_key,
