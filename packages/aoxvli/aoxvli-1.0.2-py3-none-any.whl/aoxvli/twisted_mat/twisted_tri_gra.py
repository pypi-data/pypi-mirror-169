...
class TightTtgInst(TightTbgInst):
    ...
    def plot_atoms(self, point_index_list, save_or_not=False, show_or_not=False, fig_format='.svg'):
        all_positions = []
        ...
        if not os.path.exists(save_path):
            os.mkdir(save_path)
        if save_or_not:
            plt.savefig(save_path + 'Atomic_structure_{:.2f}{}'.format(self.twist_angle, self.mat_type) + fig_format, dpi=300)
        if show_or_not:
            plt.show()
