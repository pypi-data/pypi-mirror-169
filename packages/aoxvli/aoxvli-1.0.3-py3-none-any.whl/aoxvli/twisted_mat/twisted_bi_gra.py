...
class ContiTbgInst:  # # 连续性模型的核心代码
    ...
    def multi_proc_ab_cal(self, args_list):
        ...
        plt.savefig("ab_{:.3f}_{}.png".format(self.twist_angle_conti, self.mat_type), dpi=300, facecolor='w')
        plt.close()
        return out_ab
...
class TightTbgInst:  # # 紧束缚模型核心代码
    ...
    def plot_atoms(self, save_or_not=False, show_or_not=False, fig_format='.svg'):
        layer1_atoms = []
        layer2_atoms = []
        ...
        if save_or_not:
            ...
            if not os.path.exists(save_dir):
                os.mkdir(save_dir)
            plt.savefig(save_dir + "Atomic_structure_%.2f%s" % (self.twist_angle, self.mat_type) + fig_format, dpi=300)
        if show_or_not:
            plt.show()
        plt.close()

    def plot_MBZ(self, save_or_not=False, fig_format='.svg'):
        ...
        if save_or_not:
            ...
            if not os.path.exists(save_dir):
                os.mkdir(save_dir)
            plt.savefig(save_dir + 'BZ of %.2f %s' % (self.twist_angle, self.mat_type) + fig_format, dpi=300)
        plt.close()
    ...
    def multi_proc_path(
        ...
        if isinstance(shift_energy, bool):
            ...
            if shift_energy:
                ...
                figure_title = r"Band structure for $\theta={:.3f} \degree$".format(self.twist_angle)
                save_title = 'tb_band_{:.3f}'.format(self.twist_angle)
                PubMeth.plot_energies(real(shifted_energy_list), y_range=y_range, line_type=line_type, figuretitle=figure_title, x_label_pos=x_label_pos, x_labs=x_labs, save_or_not=save_or_not, show=show_or_not, save_title=save_title, lw=lw, hold_on=hold_on, fig_size=figsize, test_mode=test_mode, selected_bds_indices=selected_bds_index_list, html_name='tb_energies_{:.4f}'.format(self.twist_angle))
            else:
                shifted_energy_list = array(total_energy_list)
        ...
        if save_or_not:
            ...
            if not os.path.exists(target_dir):
                os.mkdir(target_dir)
            plt.savefig(target_dir + "band_{:.2f}".format(self.twist_angle) + fig_format, dpi=300)
