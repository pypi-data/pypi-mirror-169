...
class BinaryUploadImpl(BinaryUploadServicer):
    ...
    def UploadChunk(
        ...
        for request in request_iterator:
            ...
            if len(binary.chunks) == binary.num_chunks and len(binary.uploaded_binary) != binary.size:
                self.binaries.pop(binary_uuid)
                raise_binary_upload_failed_error(
                    context,
                    (
                        f"Received all {binary.num_chunks} chunks for binary with UUID {request.binaryTransferUUID}"
                        f", but only {len(binary.uploaded_binary)} bytes instead of announced {binary.size} bytes"
                    ),
                )
