...
def rotate_wind(vectors, method = "PF", return_all = False):
    """
    Wrapper of rotate_wind3d
    Parameters
    ----------
    wind3D : numpy.ndarray
        A 2-D (Nx3) array with `x`, `y`, and `z` vector components,
    ...
    method : {"DR", "TR", "PF"}, optional
        ...
        planar fit.
    return_all: boolean
        Return RotatedVectors if True, rotated array only otherwise. Default: False
    Returns
    -------
    rotated_vectors:
        numpy.ndarray [0, `rotated`]
    ...
    rotated_tuple(RotatedVectors) : namedtuple
        namedtuple with (index, name in brackets):
        numpy.ndarray [0, `rotated`]
            2-D array (Nx3) Array with rotated vectors
        numpy.ndarray [1, `phi_theta`]
            ...
            to the reference frame.
    """
    rotated_tuple = rotate_wind3d(vectors, method = method)
    if return_all:
        return rotated_tuple
    else:
        rotated_vectors = rotated_tuple[0]
