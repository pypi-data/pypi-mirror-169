...
class Raw():
    ...
    def extract(self, df, col_names, rename_cols = None, nanval = None):
        dfo = df[col_names]
        if nanval:
            dfo = dfo.replace(nanval, np.nan)
        if rename_cols:
            assert type(rename_cols) == dict, "rename_cols type must be dict" 
            dfo = dfo.rename(columns = rename_cols)
        return dfo