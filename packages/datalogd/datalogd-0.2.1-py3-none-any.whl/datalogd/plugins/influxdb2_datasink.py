...
try:
    ...
    log.error("influxdb_client module not found. Install it with \"pip3 install influxdb_client\" or similar.")
else:
    ...
    class InfluxDB2DataSink(DataSink):
        ...
        specify it as the ``field_key`` parameter.
        ...
        value, specify it as the ``field_value`` parameter.
        ...
        def __init__(self, url="http://localhost:8086", token="", org="default", bucket="default", measurement="measurement", run_id=None, field_key=None, field_value=None):
            ...
            try:
                ...
                self.log.error("Unable to make connection to InfluxDB database.")
                raise
        ...
        def receive(self, data):
            ...
            for d in data:
                ...
                if self.field_key is not None and self.field_key in d.keys():
                    ...
                    k = str(d.pop("label"))
                if "type" in d.keys():
                    # Format as <type>_<id> = <value>
                    k = str(d.pop("type")) + ("_" + str(d.pop("id"))) if "id" in d.keys() else ""
                else:
                    k = "data"
                ...
                if "timestamp" in d.keys():
                    if type(d["timestamp"]) == datetime:
                        datapoint["time"] = d["timestamp"].isoformat()
                    else:
                        datapoint["time"] = d["timestamp"]
                else:
