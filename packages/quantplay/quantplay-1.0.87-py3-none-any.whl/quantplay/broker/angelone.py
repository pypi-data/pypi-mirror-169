...
class AngelOne(Broker):
        ...
        if order_book['data']:
            ...
            if 'errorcode' in order_book and order_book['errorcode'] == "AB1010":
                Constants.logger.error(
                    "{}: Can't Fetch order book because session got expired".format(self.get_userId()))
            else:
                Constants.logger.error(
                    "{}: Unknow error while fetching order book [{}]".format(self.get_userId(), order_book))
    ...
    def modify_orders_till_complete(self, orders_placed):
        ...
        while 1:
            ...
            if len(orders) == 0:
                Constants.logger.info("{} ALL orders have be completed".format(self.get_userId()))
                break
