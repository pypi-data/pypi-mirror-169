...
class HourSelectionInterimUpdate:
    @staticmethod
    def interim_avg_update(today: HourObject, tomorrow: HourObject, prices_today: list, prices_tomorrow: list) -> (HourObject, HourObject):
        avg = HourSelectionInterimUpdate._get_average_price(prices_today, prices_tomorrow)
        _today = HourSelectionInterimUpdate._set_interim_per_day(avg, prices_today, today)
        _tomorrow = HourSelectionInterimUpdate._set_interim_per_day(avg, prices_tomorrow, tomorrow, False)
        return _today, _tomorrow

    @staticmethod
    def _set_interim_per_day(avg: float, prices: list, hour_obj: HourObject, is_today: bool = True) -> HourObject:
        new_nonhours = []
        new_ok_hours = []
    ...
    def _get_average_price(prices_today: list, prices_tomorrow:list) -> float:
        ret = prices_today[0:14]
        ret[len(ret):] = prices_tomorrow[14::]
