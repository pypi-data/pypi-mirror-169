...
class SessionController(BaseController):
    ...
    def group_log_files(self, paths: List[str]):
        """
        This function groups the log file chunks that are fetched from S3. This
        assumes that files are stored under some format that ends with:

        ../<file-name>/<chunk-name>.log

        e.g. ../gcs_server.out/1660937514-b89399b5-3140-4a76-bebd-a5561cc6e576.log

        Existing download logic downloads the entire S3 bucket onto disk. Then,
        this method is called to iterate over the suffix-level directories
        (e.g. /gcs_server.out/*.log) and group them into a single log file
        (e.g. /gcs_server.out).
        """
        groups = set("/".join(path.split("/")[:-1]) for path in paths)
        for group_path in groups:
            temp_location = group_path + "_temp"
            os.rename(group_path, temp_location)
            chunks = sorted(glob(temp_location + "/*.*"))
            if len(chunks) > 0:
                # If there are any log chunks in the selected directory, and write them to
                # the appropriate result log file.
                with open(group_path, "w") as group_file:
                    for chunk_path in sorted(glob(temp_location + "/*.*")):
    ...
    async def _download_files(
        ...
        async with aiohttp.ClientSession(connector=connector) as session:
            for pos, log_detail in enumerate(log_files):
                path = os.path.join(base_folder or "", log_detail.name.lstrip("/"))
                paths.append(path)
                downloads.append(
                    asyncio.create_task(
                        self._download_file(
