...
def test_download_log_files(mock_auth_api_client) -> None:  # type: ignore
    ...
    try:
        session_controller.api_client.get_log_files_api_v2_sessions_session_id_get_log_files_post.side_effect = [
            LogdetailsResponse(
                result=LogDetails(
                    log_files=[
                        LogDetail(
                            name="logs/test1.txt/chunk_one.log",
            ...
            LogdetailsResponse(
                result=LogDetails(
                    log_files=[
                        LogDetail(
                            name="logs/test2.txt/chunk_one.log",
                            download_url="http://test.com",
                            size=len(data[1]),
                        ),
                        LogDetail(
                            name="logs/test3.txt/chunk_one.log",
                            download_url="http://test.com",
                            size=len(data[2]),
                        ),
                    ],
        ...
        with patch(
            ...
            session_controller.download_log_files(base_folder=dirpath)
        with open(f"{dirpath}/logs/test1.txt") as f:
            assert f.read().strip() == data[0].strip()
        with open(f"{dirpath}/logs/test2.txt") as f:
            assert f.read().strip() == data[1].strip()
        print("LOGS DIRECTORY:", os.listdir(f"{dirpath}/logs/"))
        assert not os.path.exists(f"{dirpath}/logs/test3.txt")
        assert (
            session_controller.api_client.get_log_files_api_v2_sessions_session_id_get_log_files_post.call_args_list
            == [
                call(
                    "latest",
                    log_download_request=LogDownloadRequest(
                        ...
                        config=LogDownloadConfig(
                            next_page_token=None, page_size=None, ttl_seconds=None,
                        ),
                    ),
                ),
                call(
                    ...
                    log_download_request=LogDownloadRequest(
                        ...
                        config=LogDownloadConfig(
                            next_page_token="foo", page_size=None, ttl_seconds=None,
                        ),
                    ),
                ),
            ]
...
def test_download_log_files_params(mock_auth_api_client) -> None:  # type: ignore
    """Test that when download_log_files is called with parameters then it invokes the get_log_files_api_v2 with the right set of parameters.
    Also ensure that the parameters are passed correctly on the second call of pagination.
    """
    session_controller = SessionController()
    data = ["".join(random.choices(string.ascii_lowercase, k=5)) for _ in range(2)]
    dirpath = tempfile.mkdtemp()
    try:
        session_controller.api_client.get_log_files_api_v2_sessions_session_id_get_log_files_post.side_effect = [
            LogdetailsResponse(
        ...
        session_controller.api_client.get_log_files_api_v2_sessions_session_id_get_log_files_post.call_args_list == [
            call(
                ...
                log_download_request=LogDownloadRequest(
                    ...
                    config=LogDownloadConfig(
                        next_page_token=None, page_size=10, ttl_seconds=20,
                    ),
                ),
            ),
            call(
                ...
                log_download_request=LogDownloadRequest(
                    ...
                    config=LogDownloadConfig(
                        next_page_token="foo", page_size=10, ttl_seconds=20,
                    ),
                ),
            ),
        ]
...
def test_download_ray_result_files(mock_auth_api_client) -> None:  # type: ignore
    """Test that the download_ray_result_files is able to download files correctly.
    Also verify handling of error response while downloading files.
    """
    session_controller = SessionController()
    ...
    dirpath = tempfile.mkdtemp()
    try:
        session_controller.api_client.get_ray_result_files_api_v2_sessions_session_id_get_ray_result_files_get.return_value = LogdetailsResponse(
            result=LogDetails(
                log_files=[
                    LogDetail(
                        name="logs/test1.txt",
        ...
        with patch(
            ...
            side_effect=[
                AsyncMockResponse(data[0], 200),
                AsyncMockResponse(data[1], 500),
            ],
        ):
            session_controller.download_ray_result_files(base_folder=dirpath)
        with open(f"{dirpath}/logs/test1.txt") as f:
            assert f.readlines() == [data[0]]
        assert not os.path.exists(f"{dirpath}/logs/test2.txt")
        session_controller.api_client.get_ray_result_files_api_v2_sessions_session_id_get_ray_result_files_get.assert_called_once_with(
            "latest", job_id=None, job_run=None
        )
    finally:
        shutil.rmtree(dirpath)


def test_download_ray_result_files_params(mock_auth_api_client) -> None:  # type: ignore
    """Test that the download_ray_result_files passed the parameters correctly to download_ray_result_files_api.
    """
    session_controller = SessionController()
    data = "".join(random.choices(string.ascii_lowercase, k=5))
    dirpath = tempfile.mkdtemp()
    try:
        ...
        with patch(
            "aiohttp.ClientSession.get", side_effect=[AsyncMockResponse(data, 200)],
        ):
            session_controller.download_ray_result_files(
                dirpath, "session_id", "job_id", "job_run"
            )
        session_controller.api_client.get_ray_result_files_api_v2_sessions_session_id_get_ray_result_files_get.assert_called_once_with(
            "session_id", job_id="job_id", job_run="job_run"
        )
    finally:
        shutil.rmtree(dirpath)
