...
class AllCls:
	...
	def fetch(self) -> List[enums.ResourceState]:
		...
		Snippet: value: List[enums.ResourceState] = driver.gprf.measurement.ploss.state.all.fetch() \n
		No command help available \n
			:return: meas_state: No help available"""
		response = self._core.io.query_str(f'FETCh:GPRF:MEASurement<Instance>:PLOSs:STATe:ALL?')
