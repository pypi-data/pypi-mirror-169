...
class AllCls:
	...
	def fetch(self) -> List[enums.ResourceState]:
		...
		Snippet: value: List[enums.ResourceState] = driver.gprf.measurement.canalyzer.state.all.fetch() \n
		...
		response = self._core.io.query_str(f'FETCh:GPRF:MEASurement<Instance>:CANalyzer:STATe:ALL?')
