...
class StateCls:
	...
	def fetch(self) -> enums.ResourceState:
		...
		Snippet: value: enums.ResourceState = driver.gprf.measurement.canalyzer.state.fetch() \n
		...
		INITiate..., STOP..., ABORt... to change the measurement state. \n
			:return: meas_state: OFF: measurement off, no resources allocated, no results RUN: measurement running, synchronization pending or adjusted, resources active or queued RDY: measurement finished"""
		response = self._core.io.query_str(f'FETCh:GPRF:MEASurement<Instance>:CANalyzer:STATe?')
