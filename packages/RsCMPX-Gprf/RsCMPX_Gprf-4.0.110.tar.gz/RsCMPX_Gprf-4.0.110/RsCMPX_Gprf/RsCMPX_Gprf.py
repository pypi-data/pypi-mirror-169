...
class RsCMPX_Gprf:
	"""712 total commands, 17 Subgroups, 0 group commands"""
	_driver_options = "SupportedInstrModels = CMX500/CMP/CMW, SupportedIdnPatterns = CMX/CMP/CMW, SimulationIdnString = 'Rohde&Schwarz,CMX,100001,4.0.110.0024'"
	...
	def __init__(self, resource_name: str, id_query: bool = True, reset: bool = False, options: str = None, direct_session: object = None):
		...
		self._core.driver_version = '4.0.110.0024'
	...
	def assert_minimum_version(min_version: str) -> None:
		...
		curr_version_list = '4.0.110.0024'.split('.')
		...
		for i in range(count):
			...
			if curr > minimum:
				break
			if curr < minimum:
				raise RsInstrException(f"Assertion for minimum RsCMPX_Gprf version failed. Current version: '4.0.110.0024', minimum required version: '{min_version}'")
