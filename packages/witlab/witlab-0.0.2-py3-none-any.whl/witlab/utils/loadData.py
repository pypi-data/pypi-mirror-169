...
class Dataset:
    ...
    stored in the `witlab/data/datasets` directory. In each dataset, subject-wise functional
    ...
    of brain regions and $t$ the number of time steps. Example datasets are included in `witlab`
    ...
    *DSIStudio*. The handling of the datasets is done by the
    ...
    transmission delays. The two example datasets currently included in `witlab` use the the 80
    ...
    def __init__(self, datasetName=None, normalizeCmats="max", fcd=False, subcortical=False):
        """
        Load the empirical data sets that are provided with `witlab`.

        Right now, datasets work on a per-subject base. A dataset must be located
        in the `witlab/data/datasets/` directory. Each subject's dataset
    ...
    def _loadSubjectFiles(self, dsBaseDirectory, subcortical=False):
        ...
        files as provided in the `witlab/data/datasets` directory. Uses `glob.glob()`
