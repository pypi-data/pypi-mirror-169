...
def add_remote_id_to_payload(kc, payload):
    ...
    for child in child_nodes:
        if child['displayName'] == payload['displayName'] and \
                child['index'] == payload['index'] and \
                child['level'] == payload['level']:
            payload['id'] = child['id']
            return payload


def insert_id_and_update(kc):
    crud_update = kc.update
    return lambda object_id, payload: crud_update(object_id, {'id':object_id, **payload})
...
def choose_update_strategy(kc, root_node):
    ...
    if is_flow_built_in(root_node):
        ...
        return insert_id_and_update(kc)
