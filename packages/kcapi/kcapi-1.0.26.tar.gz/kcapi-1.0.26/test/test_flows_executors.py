...
class TestingAuthenticationFlowsAPI(unittest.TestCase):
    ...
    def testing_create_and_update_flow(self):
        basic_flow = {
            "alias": "basic",
        ...
        for flow in flows:
            auth = self.authenticationFlow.findFirst({"key": "alias", "value": flow['alias']})
            id = auth['id']
            flow['description'] = 'updated!!'
            state = self.authenticationFlow.update(id, flow).isOk()
            self.assertTrue(state)

            mutated = self.authenticationFlow.findFirst({"key": "alias", "value": flow['alias']})
            self.assertEqual(mutated['description'],'updated!!', "We should be able to see the field updated." )
    ...
    def testing_performing_authentication_built_in_flows_updates(self):
        ...
        state = publisher.update(clients_flow, executors)
        execs = self.authenticationFlow.executions(clients_flow).all()

        self.assertTrue(state, 'We should get a True value meaning that the HTTP response has being validated.')
    ...
    def testing_performing_authentication_flows_updates(self):
        ...
        state = publisher.update(update_flow, flows)
        self.assertTrue(state, 'We should get a True value meaning that the HTTP response has being validated.')
