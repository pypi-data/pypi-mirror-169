...
def get_pre_picture(andidate_list, sentence, global_text, global_title):
    print("#####################")
    print("输入信息:")
    print(sentence)
    print(global_text)
    ...
    # 将全局文本和表头文本合并，这样，这里假设两者至多只会提取一个全局图集，且全局的优先级更高
    global_text = "，".join(global_text)
    global_title = "，".join(global_title )
    global_text = global_text + global_title
    # 如果全局存在，优先使用全局作为图集前缀
    if global_text:
        pre_list = _get_pre(global_text)
...
if __name__ == "__main__":
    ...
    sentence = "防水工程按国标11CJ23-1要求选材及施工。 防æ°´层在墙柱交接处翻起250高"
    sentence = "防水等级Ⅱ级,按国标12J201、11CJ23-1要求选材及施工 □"
    
    sentence = "1^散水^05J909^32^散1A^外墙周边^宽900mm"
    sentence = "12J1^138页-屋103^防水层为12J1-133页-1F1(3+3厚SBS 高聚物改性沥青防水卷材)"
    global_text = ""
    sentence = "12J12-25-6^用于住宅单元出入口处无障碍坡道"
    sentence = "1^地面^12YJ1,地101F1^卫生间等有水房间"
    # sentence = ["名 称", "编 号 (参L13J1)
