...
class DBAPI(BaseDB):
    ...
    def get_posturls_to_getcontent(self, uid: int = 1):
        """get the urls which to get content"""
        return (
            self.session.query(PostURL.post_url)
            .filter(PostURL.uid >= uid)
            .filter(PostURL.content_status == None)
            .all()
        )
