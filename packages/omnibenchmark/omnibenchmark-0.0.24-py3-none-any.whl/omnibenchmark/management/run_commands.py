...
def revert_run(out_files: Optional[List[str]] = None, in_files: Optional[List[str]] = None, plan_id: Optional[str] = None, dataset_name: Optional[str] = None, remove: bool = True):
    ...
    if len(activities) > 0:
        out_fis=flatten([[act_out.path for act_out in act.generated_outputs] for act in activities])
    
    if out_files is not None:
        activities = activities + flatten([Activity.filter(output = out_fi) for out_fi in out_files])
    ...
    if dataset_name is not None:
        unlink_dataset_files(out_files = out_fis, dataset_name = dataset_name, remove = remove)

    #revert activities
    # Remove plan only if there is no other activity related to this? Check renku code!
    activity_ids=list(set([act.id for act in activities]))
    [omni_wflow.renku_workflow_revert(activity_id= act_id, plan = True) for act_id in activity_ids]
