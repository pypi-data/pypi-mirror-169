...
def unlink_dataset_files(out_files: List[str], dataset_name: str, remove:bool = True):
    ...
    if dataset_name not in name_list:
        raise InputError(
            "Dataset {dataset_name} does not exist in this project."
        )
    dataset = [data for data in datasets if data.name == dataset_name][0]
    ...
    [renku_unlink_from_dataset(name = dataset_name, include=out_url) for out_url in out_urls if out_url in dataset_files]
    if remove:
