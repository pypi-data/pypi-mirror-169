...
from web3_eth._utils.ens import (
    is_ens_name,
)
from web3_eth.exceptions import (
    InvalidAddress,
    ManifestValidationError,
    NameNotFound,
    PMError,
)
from web3_eth.module import (
    Module,
)

# Package Management is still in alpha, and its API is likely to change, so it
# is not automatically available on a web3_eth instance. To use the `PM` module,
# please enable the package management API on an individual web3_eth instance.
#
...
class ERC1319Registry(ABC):
    ...
    accessed directly, but rather through the methods on ``web3_eth.pm``.
    ...
    with `web3_eth.pm.PM`. Any implementation specific logic should be
    ...
    def __init__(self, address: Address, w3: Web3) -> None:
        """
        Initializes the class with the on-chain address of the registry, and a web3_eth
        instance connected to the chain where the registry can be found.

        Must set the following properties...

        * ``self.registry``: A `web3_eth.contract` instance of the target registry.
        * ``self.address``: The address of the target registry.
        * ``self.w3``: The *web3_eth* instance connected to the chain where the
...
class PM(Module):
    ...
    def set_registry(self, address: Union[Address, ChecksumAddress, ENS]) -> None:
        """
        Sets the current registry used in ``web3_eth.pm`` functions that read/write
    ...
    def deploy_and_set_registry(self) -> ChecksumAddress:
        ...
        ``web3_eth.pm.registry``.

        To tie your registry to an ENS name, use web3_eth's ENS module, ie.
    ...
    def release_package(
        ...
        Requires ``web3_eth.PM`` to have a registry set. Requires
        ``web3_eth.eth.default_account`` to be the registry owner.
    ...
    def _validate_set_registry(self) -> None:
        try:
            ...
            raise PMError(
                "web3_eth.pm does not have a set registry. "
                "Please set registry with either: "
                "web3_eth.pm.set_registry(address) or "
                "web3_eth.pm.deploy_and_set_registry()"
            )
        if not isinstance(self.registry, ERC1319Registry):
            raise PMError(
                "web3_eth.pm requires an instance of a subclass of ERC1319Registry "
                "to be set as the web3_eth.pm.registry attribute. Instead found: "
    ...
    def _validate_set_ens(self) -> None:
        if not self.w3:
            raise InvalidAddress(
                "Could not look up ENS address because no web3_eth " "connection available"
            )
        elif not self.w3.ens:
            raise InvalidAddress(
                "Could not look up ENS address because web3_eth.ens is " "set to None"
            )
