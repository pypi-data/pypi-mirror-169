...
PRODUCTS = {
    ...
    "Firmware Update Utility (CLI)": '-3',
    "Firmware Update Utility (GUI)": '-4',
}
...
def get_category(key):
    if is_driver(key):
        return 'driver'
    elif key == "Firmware Update Utility (CLI)" or \
         key == "Firmware Update Utility (GUI)":
        return 'utility'
    else:
        return 'software'
