...
def PyJsHoisted_toGreeklish_(text, this, arguments, var=var):
    ...
    var.registers(['replacements', 'i', 'fixCase', 'stringToSet', 'grLetters', 'grCaps', 'viSet', 'replacement', 'greekSet', 'expression', 'engLetters', 'text'])
    @Js
    ...
    @Js
    def PyJs_anonymous_1_(PyJsArg_2430_, index, this, arguments, var=var):
        var = Scope({'$0':PyJsArg_2430_, 'index':index, 'this':this, 'arguments':arguments}, var)
        var.registers(['c1', 'replacement', 'bi', 'c2', '$0', 'index'])
    ...
    var.put('text', var.get('text').callprop('replace', var.get('expression'), PyJs_anonymous_1_))
    return var.get('text')
    pass
    pass
...
def PyJs_anonymous_0_(this, arguments, var=var):
    ...
    var.registers(['prevText', 'inputElement', 'transform'])
    @Js
    def PyJsHoisted_transform_(this, arguments, var=var):
        var = Scope({'this':this, 'arguments':arguments}, var)
        var.registers(['text'])
        var.put('text', var.get('document').callprop('getElementById', Js('el')).get('value'))
        if (var.get('prevText')!=var.get('text')):
            var.put('prevText', var.get('text'))
            var.get('document').callprop('getElementById', Js('en')).put('value', var.get('toGreeklish')(var.get('text')))
        var.get('M').callprop('textareaAutoResize', var.get('document').callprop('getElementById', Js('en')))
    PyJsHoisted_transform_.func_name = 'transform'
    var.put('transform', PyJsHoisted_transform_)
    var.put('inputElement', var.get('document').callprop('getElementById', Js('el')))
    var.get('inputElement').callprop('focus')
    var.get('inputElement').put('onkeyup', var.get('transform'))
    var.get('inputElement').put('onmouseup', var.get('transform'))
    var.put('inputElement', var.get(u"null"))
    var.put('prevText', var.get(u"null"))
    var.get('transform')()
    pass
PyJs_anonymous_0_._set_name('anonymous')
var.get('window').put('onload', PyJs_anonymous_0_)
pass
pass
