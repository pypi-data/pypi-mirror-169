...
from lattice.elastic.events import Event, EventSource, record
...
class SimpleElasticAgent(ElasticAgent):
    ...
    def _invoke_run(self, role: str = DEFAULT_ROLE) -> RunResult:
        ...
        breeze_store = rdzv_handler._rdzv_impl.setup_global_kv_store()

        while True:
            assert self._worker_group.state != WorkerState.INIT
            time.sleep(monitor_interval)
            check_for_preemption(breeze_store)
            ...
            if state == WorkerState.SUCCEEDED:
                ...
                breeze_store.set('mod-save', '0')
                breeze_store.set('opt-save', '0')
