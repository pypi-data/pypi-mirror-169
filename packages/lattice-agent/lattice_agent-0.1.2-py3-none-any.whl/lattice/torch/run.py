...
def get_args_parser() -> ArgumentParser:
    ...
    # Arguements to pass to the worker
    parser.add_argument(
        "--checkpoint_endpoint",
        action=env,
        required=True,
        help="Path to directory or URI indicating where to store checkpoints",
    )
    parser.add_argument(
        "--steps_per_checkpoint",
        action=env,
        default=20,
        help="The number of training steps run before saving a checkpoint",
    )
