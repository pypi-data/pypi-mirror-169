...
class Loaf:
    ...
    def select(self, select, fromm="", where="", order="", limit=""):
        ...
        query += f" FROM {parse(fromm)}" if fromm != "" else ""
        query += f" WHERE {parse(where)}" if where != "" else ""
        query += f" ORDER BY {parse(order)}" if order != "" else ""
        query += f" LIMIT {parse(limit)}" if limit != "" else ""
        query += ";"
        # Execute the query.
        self.cursor.execute(query)
        return self.cursor.fetchall()
