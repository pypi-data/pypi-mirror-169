...
class JupyterServer():
    ...
    def locate_jupyter(self, exp_version):
        version = ''
        version, _ = self._proc.run_cmd(['jupyter-notebook', '--version'], err_handling=self._proc.CAPTURE)
        self._log.log(f'Searching for {exp_version} and found {version}')

    def start_jupyter(self, start_directory=""):
        self._log.log(f'Starting Jupyter')
        ...
        try:
            ...
            self._jupyter_app = self._proc.start(cmd, err_handling=self._proc.LOG,
                                cwd=self._start_directory, creationflags=creationflags,
                                env=my_env)
            attempts = 20
        ...
        while self._web_address is None and attempts > 0:
            ...
            # osx
            output = \
                self._jupyter_app.stderr.readline().strip()
            ...
            if "http://" in output:
                ...
                self._log.log(
                    "Jupyter Server found at %s" % self._web_address)
    ...
    def stop_jupyter(self):
        ...
        result, _ = self._proc.run_cmd(['jupyter', 'notebook', 'stop'], err_handling=self._proc.CAPTURE)
