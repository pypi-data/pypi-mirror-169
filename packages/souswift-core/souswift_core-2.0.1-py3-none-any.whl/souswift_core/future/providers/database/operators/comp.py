...
def between(
    attr: ColumnElement, expected: tuple[SortableTypeDef, SortableTypeDef]
) -> base.ComparationOutput:
    left, right = expected
    return attr.between(left, right)


def like(attr: ColumnElement, expected: str) -> base.ComparationOutput:
    return attr.like(f'%{expected}%')


def ilike(attr: ColumnElement, expected: str) -> base.ComparationOutput:
...
def isnull(attr: ColumnElement, expected: bool) -> base.ComparationOutput:
