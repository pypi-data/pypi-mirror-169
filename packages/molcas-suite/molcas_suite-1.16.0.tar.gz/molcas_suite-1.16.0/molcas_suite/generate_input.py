...
class MolcasProg:
    ...
    def __str__(self):
        ...
        return '\n'.join(
            ...
            [indent + "{}= {}".format(key, val)
             for key, val in self.kwargs.items() if val is not None])
...
class Casscf(MolcasProg):
    ...
    def __str__(self):
        ...
        casscf_definition = [indent + f"Nactel= {self.nactel} 0 0",
                             indent + f"RAS2= {self.ras2}"]

        return '\n'.join([super().__str__(), *casscf_definition])
...
class Rassi(MolcasProg):
    ...
    def __str__(self):
        ...
        if self.jobiph is not None:
            jobiph_section = [
                indent + "Nr of JobIph= {} {}".format(
                    str(len(self.jobiph)),
                    " ".join([str(root) for root in self.jobiph])
                ),
                *[indent + " ".join([str(r) for r in range(1, root + 1)])
        ...
        if self.properties is None or len(self.properties) == 0:
            ...
            prop_section = [indent + line for line in [
                "Prop", str(len(self.properties)), *self.properties]]

        return '\n'.join([super().__str__(), *jobiph_section, *prop_section])
...
class SingleAniso(MolcasProg):
    ...
    def __str__(self):
        ...
        quax_section = indent + "QUAX= {}".format("3\n{}\n{}\n{}".format(
            *[" ".join(map(str, row)) for row in self.quax])
            if self.quax else 1)

        return '\n'.join([super().__str__(), quax_section])
...
def generate_input(labels, coords, central_atom, charge, n_active_elec,
    ...
    sections.append(Rassi(
        ...
        IPHN=" ".join([f"{i}_IPH" for i in range(1, len(spin_list) + 1)]),
        **rassi_extra[1]))
