...
def _complete_resource_node_name(ctx: typer.Context, args: List[str], incomplete: str):
    err_console.print(ctx)
    err_console.print(ctx.params)
    err_console.print(args)
    
    service = ctx.params.get("service", False)
    if not service:
        err_console.print("no service")
    service_node = ServiceNode(service, session)
    if not service_node:
        err_console.print("no service node")
        return [ ]
    resource_nodes = service_node.get_resource_nodes()
    if not resource_nodes:
        err_console.print("no resource node")
        return []
    a = [_rn.name for _rn in resource_nodes]
    err_console.print(a)
    return a
