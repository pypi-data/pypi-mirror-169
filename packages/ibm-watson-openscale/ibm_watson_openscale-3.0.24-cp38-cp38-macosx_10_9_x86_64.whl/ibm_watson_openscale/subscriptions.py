...
class Subscriptions(BaseSubscriptions):
    ...
    def create_subscription_using_training_data(self, subscription_name, datamart_id, service_provider_id, model_info, sample_csv, spark_credentials, data_warehouse_connection, payload_table, feedback_table, scored_training_data_table, managed_by):
        """
            ...
            :param dict model_info: Information about the model which needs the subscription.
                ...
                model_info = {
                    ...
                    "url":"scoring url", # This is required if explainability needs to be enabled
        ...
        if (managed_by == "self"):
            ...
            self._validate_provider_for_batch(service_provider_id)

            # Take feature and catogerical field from user if provided.
            user_feature_field = get(model_info,"feature_columns")
            ...
            for field in fields:
                if field['name'] != label_column:
                    if user_feature_field == None or len(user_feature_field) == 0:
                        feature_fields.append(field['name'])
                    field["metadata"]["modeling_role"] = "feature"
                    if field['type'] == "string":
                        if user_categorical_field == None or len(user_categorical_field) == 0:
                            categorical_fields.append(field['name'])
                        field["metadata"]["measure"] = "discrete"
                else:
                    field["metadata"]["modeling_role"] = "target"

            # Override feature and catogerical fields if provided by user
            if user_feature_field != None and len(user_feature_field) != 0:
                feature_fields = user_feature_field
                # Update fields used to generate input and output data schema
                allFields = copy.deepcopy(feature_fields)
                allFields.append(label_column)
                newFields = [x for x in fields if x["name"] in allFields]
                fields = newFields
            ...
            try:
                ...
                problem_type = None
                if  model_info["model_type"]=="binary":
                    problem_type = ProblemType.BINARY_CLASSIFICATION
                elif  model_info["model_type"]=="multiclass":
                    problem_type = ProblemType.MULTICLASS_CLASSIFICATION
                ...
                asset_deployment = AssetDeploymentRequest(
                    ...
                    scoring_endpoint = ScoringEndpointRequest(url=get(model_info,"url"))
                )
                copiedFields = copy.deepcopy(fields)
                input_fields = [x for x in copiedFields if x["name"] != label_column]
                input_data_schema = {"fields" : input_fields}
                input_data_schema["type"] = responseSchema.type
                output_fields = copy.deepcopy(input_fields)
                prediction = {
                        "metadata": {
                            "modeling_role":"prediction"
                        },
                        "name": get(model_info,"prediction"),
                        "nullable": True,
                        "type": "string"
                    }
                probability = {
                        ...
                        "name": get(model_info,"probability"),
                        "nullable": True,
                        "type": { "containsNull": True, "elementType": "double", "type": "array" }
                    }
                output_fields.append(prediction)
                ...
                asset_properties_request = AssetPropertiesRequest(
                    label_column=get(model_info,"label_column"),
                    probability_fields=[get(model_info,"probability")],
            ...
            training_data_scheme = responseSchema.to_dict()
    ...
    def _validate_provider_for_batch(self, provider_id):
        provider = self._ai_client.service_providers.get(provider_id).result
