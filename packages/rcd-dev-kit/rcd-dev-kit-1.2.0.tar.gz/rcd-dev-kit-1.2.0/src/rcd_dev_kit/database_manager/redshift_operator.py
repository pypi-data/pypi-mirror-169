...
def data_warehouse_quality_report(report_file_name: str = "datawarehouse_quality_report"):
    ...
    quality_report.write("This report aims to help the developers cleaning/improving their tables on our Data Warehouse.\n"
    ...
    for user_i in ddl_model["owner_user"].unique():
        ...
        redshift_df_info = pd.merge(redshift_df, tables_info_df, left_on=['schema_name', 'table_name'], right_on=['schema', 'table'])
        tables_without_descr_df = redshift_df[(redshift_df["table_description"] == "") | (redshift_df["columns_description"] == "")][["schema_name", "table_name"]]

        tables_contains_last_update_df = redshift_df[redshift_df["table_description"].str.contains("Last Update", case=False)]
        ...
        tables_last_update_df["last_update"] = tables_contains_last_update_df["table_description"].apply(
                lambda x: re.findall(r'.*\n\s*last update\s*:(.*)\n', x.lower())[0].strip().replace(" ","").replace("–","/").replace("-", "/").replace(".", "").split("/")[-1])
        ...
        tables_last_update_old_df = tables_last_update_int_df[tables_last_update_int_df["last_update"].astype('int') < 2022]
        ...
        quality_report.write(f"\t - Tables with less than 5 rows (TO VERIFY IF IT'S INDEED CONSISTENT AND NOTHING IS WRONG):\n")
        ...
        quality_report.write(f"\t - Tables whose description has a 'Last Update' topic dating longer than 2022 or has a non-standard object. "
        ...
        if tables_last_update_df.__len__() > 0:
            tables_last_update_old_and_wrong_df = pd.concat([tables_last_update_nonint_df, tables_last_update_old_df], ignore_index=True)
            tables_last_update_old_and_wrong_df = tables_last_update_old_and_wrong_df[["schema_name", "table_name", "last_update"]]
...
def send_metadata_to_redshift(table_name: str, database: Optional[str] = None, file_path: str = "table_metadata.json", first_data: str = None, last_data: str = None,):
    """
    ...
    :param first_data: The most ancient date present regarded by your data -> [YYYY-MM-DD].
    :param last_data: The most recent date present regarded by your data -> [YYYY-MM-DD].
    ...
    if (not first_data) or (not last_data):
        raise NameError("first_data or last_data are missing. "
                        "Please, pass these values under the following format: YYYY-MM-DD.")
    ...
    if send_metadata:
        ...
        if table != "global_sources_track_table":
            warnings.warn("🥲 It seems that you are not sending this table with its metadata. "
                          ...
                          "For the .json template, please check https://github.com/OpenInnovationProgram/rcd-dev-kit/blob/main/table_metadata.example.json .", ResourceWarning)
