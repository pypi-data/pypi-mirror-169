...
def to_azure_blob_storage(
    ...
    from ..client import Client, DataBlob, DataSource, Model, Prediction
    ...
    data_source = data_blob.to_datasource(
        file_type="{fill in here}",
        index_column="{fill in here}",
        sort_by="{fill in here}"
    )
    data_source.progress_bar()

    # Training a model
    model = data_source.train(
        client_column="{fill in here}",
        target_column="{fill in here}",
        target="{fill in here}",
        predict_after=timedelta(hours=int("{fill in here}")),
    )
    model.progress_bar()
