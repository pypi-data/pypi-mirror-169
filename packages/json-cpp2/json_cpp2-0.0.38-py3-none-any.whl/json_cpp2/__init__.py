    ...
    def __get_descriptor__(self):
        ...
        for member_name in members:
            ...
            if member_type is bool:
                jd.set_bool_member(self.member_name, member_value)
            elif member_type is int:
                jd.set_int_member(self.member_name, member_value)
            elif member_type is float:
                jd.set_float_member(self.member_name, member_value)
            elif member_type is str:
                jd.set_string_member(self.member_name, member_value)
            elif issubclass(member_type, JsonObject2):
                jd.set_object_member(self.member_name, member_value.__get_descriptor__())
            else:
                raise "type not supported"
        return jd
