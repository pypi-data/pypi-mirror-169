    ...
    def from_json(self, json_string: str):
        ...
        for member in members:
            ...
            if member.python_type == "bool":
                member_val = jd.get_bool_member(member.name)
            elif member.python_type == "int":
                member_val = jd.get_int_member(member.name)
            elif member.python_type == "float":
                member_val = jd.get_float_member(member.name)
            elif member.python_type == "string":
                member_val = jd.get_string_member(member.name)
            else:
                raise "type not supported"
            setattr(self, member.name, member_val)
