...
def remote_job_control(protocol_triad, batch_job_code, fs_locations, tasks_per_job,
    ...
    if not only_gather:
        if protocol != 'local':
            ...
            p.wait()
            time.sleep(20)
            print("PRINT THIS FUCK")
            p = subprocess.Popen(["ssh",remote_machine,"cat",fs_locations["runtime_exec"] + 'submit.log'])
            p.wait()
        else:
...
#                        print("YE", line)
            ...
            if not (mng_times and mac_names):
                ...
                if not (all_completed) and (nocache_count < NOCACHE_COUNT_THR):
                    print("THERE ARE INCOMPLETE JOBS. ROUND", nocache_count)
                    is_over = False
                    continue
...
#            print(snapshot)
        ...
        print("\n\n\n\nTEST RESCHEDULE: FORCE RESCHEDULING OF 5th-9th JOBS\n\n\n\n")
        resc_job_data = job_data[4:9]
        print(resc_job_data)
        print("\n\n\n\n\n\n\n")
